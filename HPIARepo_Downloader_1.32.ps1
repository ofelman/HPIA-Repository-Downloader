...
Function CreateForm {
    ...
    $CommonRepoRadio_Click = {
        ...
        CMTraceLog -Message "$($IniFile) - modifying setting: ''$($replace)''" -Type $TypeNorm
        (Get-Content $IniFIleFullPath) | Foreach-Object {if ($_ -match $find) {$replace} else {$_}} | Set-Content $IniFIleFullPath
    ...
    $PathsGroupBox.Size = New-Object System.Drawing.Size(400,60)                                # (width, height)
    ...
    $HPIAPathButton.Text = "Update HPIA"
    $HPIAPathButton.AutoSize = $true
    $HPIAPathButton.location = New-Object System.Drawing.Point(($LeftOffset+330),($TopOffset-8))   # (from left, from top)
    ...
    $HPIAPathField.location = New-Object System.Drawing.Point(($LeftOffset+420),($TopOffset-5)) # (from left, from top)
    $HPIAPathField.Size = New-Object System.Drawing.Size(290,$FieldHeight)                      # (width, height)
    $HPIAPathField.ReadOnly = $true
    $HPIAPathField.Name = "HPIAPath"
    $HPIAPathField.BorderStyle = 'FixedSingle'                                                  # 'none', 'FixedSingle', 'Fixed3D (default)'
    ...
    $dataGridView.Columns[($dataGridView.ColumnCount-1)].MinimumWidth = 200
    ...
    $TextBox.location = New-Object System.Drawing.Point($LeftOffset,($TopOffset+330))            # (from left, from top)
    $TextBox.Size = New-Object System.Drawing.Size(($FormWidth-60),240)             # (width, height)
    ...
    $DebugCheckBox.location = New-Object System.Drawing.Point(($LeftOffset+250),($FormHeight-45))   # (from left, from top)
