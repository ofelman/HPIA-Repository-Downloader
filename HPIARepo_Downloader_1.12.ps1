...
Function init_repoFolder {
    ...
    CMTraceLog -Message "> init_repoFolder" -Type $TypeNorm
...
Function invoke_Sync {
    ...
    CMTraceLog -Message "> invoke_Sync " -Type $TypeNorm
...
Function get_filters {
    ...
    if ( Test-Path $RepoShareMain ) {
            ...
            foreach ( $filterSetting in $lProdFilters ) {
                ...
                for ( $i = 0; $i -lt $pDataGridList.RowCount; $i++ ) {
                    ...
                    if ( $filterSetting.platform -eq $pDataGridList[1,$i].value) {
                        ...
                        $pDataGridList[8,$i].Value = $RepoShareMain
                    }
        ...
        if ( $DebugMode ) { CMTraceLog -Message 'Main Repo Host Folder ''$($RepoShareMain)'' does NOT exist' -Type $TypeDebug }
