...
Function Create_HPModelsList {
    ...
    $lModelsListINIFile = $IniFIleFullPath
    if ( $DebugMode ) { CMTraceLog -Message '> Create_HPModelsList' -Type $TypeNorm }
    ...
    $lResponse = [System.Windows.MessageBox]::Show("Update INI file with this repository filter models?","Update INI File",4)    # 1 = "OKCancel" ; 4 = "YesNo"
    if ( $lResponse -eq 'Yes' ) {
        ...
        if ( Test-Path $lModelsListINIFile ) {
            ...
            $Script:RepoShareCommon = $pRepository

            $CommonButton.Checked = $true                          # set the visual default from the INI setting
            $ShareButton.Checked = $false 

            $SharePathSingleTextField.Text = "$pRepository"
            $SharePathSingleTextField.BackColor = $BackgroundColor
            $SharePathTextField.BackColor = ""

            $lfindValue = "^[\$]RepoShareCommon"
            $lreplaceValue = "`$RepoShareCommon = ""$pRepository"""
            Mod_INISetting $IniFIleFullPath $lfindValue $lreplaceValue "$($IniFile) - changing setting: ''$($lreplaceValue)''"

            # ... and also mod the INI file to default to: 'CommonRepo = $True'

            $lfindValue = "^[\$]CommonRepo"
            $lreplaceValue = "`$CommonRepo = `$$true"
            Mod_INISetting $IniFIleFullPath $lfindValue $lreplaceValue "$($IniFile) - changing setting: ''$($lreplaceValue)''"

        } else {
            CMTraceLog -Message " ... INI file not found" -Type $TypeWarn
...
Function Import_Repo {
    ...
    if ( $DebugMode ) { CMTraceLog -Message '> Import_Repo' -Type $TypeNorm }

    # let's find the repository to import

    $lbrowse = New-Object System.Windows.Forms.FolderBrowserDialog
    $lbrowse.SelectedPath = $RepoShareCommon        # start with the repo listed in the INI file
    $lbrowse.ShowNewFolderButton = $false
    ...
    if ($lbrowse.ShowDialog() -eq "OK") {
        ...
        if ( (Test-Path "$($lRepository)\.Repository") -and (Test-Path "$($lRepository)\.Repository\repository.json") ) {

            # first clear out existing rows from GUI grid
            for ( $i = $pDataGridList.RowCount; $i -gt 0; $i-- ) {
                $pDataGridList.Rows.RemoveAt($i-1)
            } # for ( $i = 0; $i -lt $pDataGridList.RowCount; $i++ )            # see if the repository was configured for HPIA
            
            set-location $lRepository
            
            # now, obtain the CMSL filters from the repo
            $lProdFilters = (get-repositoryinfo).Filters
            # and parse to get the supported platforms in the repository
            $lRepoPlatforms = $lProdFilters.platform | Get-Unique

            # next, make sure we have enough rows in the grid to populate with the filters
            for ( $i = 0 ; $i -lt $lRepoPlatforms.count ; $i++) {
                $lProdName = Get-HPDeviceDetails -Platform $lRepoPlatforms[$i]
                $row = @( $true, $lRepoPlatforms[$i], $lProdName[$i].Name )         
                [void]$pDataGridList.Rows.Add($row)
            } # for

            # Finally, populate the filters from the repository
            Get_Filters $pDataGridList $False
            
            # ... create a Models list and modify the INI.ps1 config file w/ imported models
            Create_HPModelsList $pDataGridList $lRepository
    ...
    for ( $i = 0; $i -lt $pDataGridList.RowCount; $i++ ) {
        ...
        if ( $dataGridView[0,$i].Value ) {
            ...
            if ( $pOSVersion -in ($lOSList).OperatingSystemRelease ) {
                ...
                CMTraceLog -Message "-- OS NOT supported: $($lPlatform)/$($lPlatformName)" -Type $TypeError
            } #
        } # if ( $dataGridView[0,$i].Value )  
...
Function CreateForm {
    ...
    #----------------------------------------------------------------------------------
    if ( $DebugMode ) { Write-Host 'creating Shared field' }
    ...
    if ( $DebugMode ) { Write-Host 'creating Common repo field' }
    ...
    if ( $DebugMode ) { Write-Host 'creating Shared/Common radio buttons' }
    ...
    $ListFiltersdButton.Location = New-Object System.Drawing.Point(($LeftOffset+80),($FormHeight-410))    # (from left, from top)
    ...
    #----------------------------------------------------------------------------------
    # Add a Load From Repo button
    #----------------------------------------------------------------------------------
    if ( $DebugMode ) { Write-Host 'creating a Import Repo button' }
    $ImportRepodButton = New-Object System.Windows.Forms.Button
    $ImportRepodButton.Location = New-Object System.Drawing.Point(($LeftOffset+180),($FormHeight-410))    # (from left, from top)
    $ImportRepodButton.Text = '{Import Repo}'
    $ImportRepodButton.AutoSize=$true

    $ImportRepodButton_Click={
        Import_Repo  $dataGridView
    } # $ListFiltersdButton_Click={

    $ImportRepodButton.add_Click($ImportRepodButton_Click)

    $CM_form.Controls.Add($ImportRepodButton)
