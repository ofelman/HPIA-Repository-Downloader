    ...
    if ( $pmodelChecked ) {
        ...
        foreach ( $cat in $FilterCategories ) {
            ...
            if ( $datagridview.Rows[$pRow].Cells[$cat].Value ) {
                CMTraceLog -Message  "... adding filter: -platform $($pModelID) -os win10 -osver $OSVER -category $($cat) -characteristic ssm" -Type $TypeNoNewline
...
[CmdletBinding()]
    ...
    $pCurrentLoc = Get-Location
    CMTraceLog -Message "> Sync_Repository " -Type $TypeNorm
...
[CmdletBinding()]
    ...
    $pCurrentLoc = Get-Location
    CMTraceLog -Message "> Sync_Repositories " -Type $TypeNorm
    if ( $DebugMode ) { CMTraceLog -Message "... for OS version: $($OSVER)" -Type $TypeDebug }
...
Function get_filters {
    ...
    if ( Test-Path $lMainRepo ) {
        CMTraceLog -Message '... Refreshing Grid for selected Models' -Type $TypeNorm
...
Function CreateForm {
    ...
    $buttonSync.add_click( {
        ...
        if ( $Script:OSVER -in $Script:OSVALID ) {
            ...
            if ( $Script:SingleRepo ) {
                Sync_Repository $dataGridView $lCheckedListArray
            } else {
    ...
    $ShareButton.Add_Click( $singleRepoRadio_Click )
    $CommonButton.Add_Click( $singleRepoRadio_Click )
