<#
    ...
    Versionj 1.62 - scriptable initial version
#>
<#
    Args
        -help|-h
...
if ( $help ) {
    ...
    "`nHPIARepo_Downloader_1.60.ps1 [[-Sync] [-ListFilters] [-inifile .\<filepath>\HPIARepo_ini.ps1] [-RepoStyle common|individual] [-products '80D4,8549,8470'] [-NoIniSw] [-ShowActivityLog]]`n"
...
Function download_softpaqs_by_name {
    ...
    $HPModelsTable  | 
        ForEach-Object {
            if ( ($_.ProdCode -match $pProdCode) -and $pChecked ) {                              # let's match the Model to download for
                ...
                ForEach ( $Softpaq in $_.SqName ) {
                    ...
                    Try {
                        ...
                        $lListret = (Get-SoftpaqList -platform $pProdCode -os $OS -osver $OSVER -characteristic SSM) 6>&1 | Where-Object { $_.Name -match $Softpaq } | 
                            ForEach-Object { 
                                if ($_.SSM -eq $true) {
...
[CmdletBinding()]
    ...
    #--------------------------------------------------------------------------------
    # ini.ps1 $Script:KeepFilters allows the use of existing Repo filters, while adding new ones
    #   for example, to have 2 OS Versions of Softpaqs in the repository
    #--------------------------------------------------------------------------------
    
    if ( $Script:KeepFilters ) {
        CMTraceLog -Message  "... Keeping existing filters in: $($pModelRepository)" -Type $TypeNorm
    } else {
        CMTraceLog -Message  "... Removing existing filters in: $($pModelRepository)" -Type $TypeNorm
...
[CmdletBinding()]
    ...
    for ( $i = 0; $i -lt $pModelsList.RowCount; $i++ ) {
        ...
        if ( $i -in $pCheckedItemsList ) {
            ...
            download_softpaqs_by_name  $lMainRepo $lModelID $pModelsList.Rows[$i].Cells['INI S/W'].Value
...
[CmdletBinding()]
    ...
    for ( $i = 0; $i -lt $pModelsList.RowCount; $i++ ) {
        ...
        if ( $i -in $pCheckedItemsList ) {
            ...
            # update specific Softpaqs as defined in INI file for the current model
            #--------------------------------------------------------------------------------
            download_softpaqs_by_name  $lTempRepoFolder $lModelID $pModelsList.Rows[$i].Cells['INI S/W'].Value
...
Function CreateForm {
    ...
    $CheckAll_Click={
        ...
        if ( $state ) {
            for ($i = 0; $i -lt $dataGridView.RowCount; $i++) {
                ...
                $dataGridView.Rows[$i].Cells['INI S/W'].Value = $state
            }
        } else {
    ...
    #----------------------------------------------------------------------------------
    if ( $DebugMode ) { Write-Host 'creating All INI Software column' }
    $CheckBoxINISoftware = New-Object System.Windows.Forms.DataGridViewCheckBoxColumn
    $CheckBoxINISoftware.Name = 'INI S/W'
    $CheckBoxINISoftware.width = 40
    ...
    # columns= 0(row selected), 1(ProdCode), 2(Prod Name), 3-6(categories, 7(INI S/W), 8(repository path)
    #----------------------------------------------------------------------------------
    $CellOnClick = {
            ...
            switch ( $column ) {
                ...
                0 {                                                               # need to reset all categories
                    if ( $CellNewState ) {
                        ...
                        $dataGridView.Rows[$row].Cells['INI S/W'].Value = $true
                    } else {                               
                        $datagridview.Rows[$row].Cells['INI S/W'].Value = $false      # reset the 'All' column
                ...
                Default {                                                         
                    # here to deal with clicking on a category cell or 'INI S/W'
                    if ( -not $CellNewState ) {
                        foreach ( $cat in $FilterCategories ) {
                        ...
                        # confirm if INI S/W is selected/checked 
                        if ( ($column -ne 7) -and ($datagridview.Rows[$row].Cells['INI S/W'].Value) ) {
                            $CellNewState = $true
                        }
                    } # if ( -not $CellNewState )
...
if ( $MyInvocation.BoundParameters.count -eq 0) {
    ...
    if ( $PSBoundParameters.Keys.Contains('newLog') ) {
        Backup_Log $LogFile } 
    ...
    if ( $PSBoundParameters.Keys.Contains('IniFile') ) { 
        '-iniFile: ' + $inifile }
    if ( $PSBoundParameters.Keys.Contains('RepoStyle') ) { 
        if ( $RepoStyle -match 'Common' ) { 
            '-RepoStyle: ' + $RepoStyle
            $CommonRepo = $true } else { $CommonRepo = $false }  } 
    if ( $PSBoundParameters.Keys.Contains('Products') ) { 
        "-Products: $($Products)" }
    if ( $PSBoundParameters.Keys.Contains('ListFilters') ) { 
        list_filters }
    if ( $PSBoundParameters.Keys.Contains('NoIniSw') ) { 
        '-NoIniSw' }
    if ( $PSBoundParameters.Keys.Contains('showActivityLog') ) {
        $showActivityLog = $true } 
    if ( $PSBoundParameters.Keys.Contains('Sync') ) { 
        sync_repos }
