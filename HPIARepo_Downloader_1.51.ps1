...
<#
    Function Backup_Log
        Rename the file name in argument with .bak extention
        Args:
            $pRepoFOlder: file to rename (as backup)
#>
Function Backup_Log {
    [CmdletBinding()]
    ...
    if ( $pLogFileFullPath ) {
        ...
        #Write-Host $lLogFilePath
        for ($i=0; $i -lt 100; $i++ ) {
            $lNewLogFilePath = $pLogFileFullPath+'.bak'+$i.ToString()
