...
$ScriptVersion = "1.81 (August-1-2021)"
...
[CmdletBinding()]
    ...
    for ( $item = 0; $item -lt $pModelsList.RowCount; $item++ ) {
        ...
        if ( $item -in $pCheckedItemsList ) {
            ...
            #--------------------------------------------------------------------------------
            download_softpaqs_by_name  $pCommonRepo $lModelID $pModelsList.Rows[$item].Cells['AddOns'].Value
...
Function get_filters {
    ...
    if ( $Script:v_CommonRepo ) {
        if ( -not ([string]::IsNullOrWhiteSpace($Script:v_Root_CommonRepoFolder)) ) {
            $lPathIsValid = $True
            Read_CommonRepofilters $pDataGrid $Script:v_Root_CommonRepoFolder $pRefreshGrid
        }
    } else {
...
Function Update_INIModels {
    ...
    for ( $i = 0; $i -lt $pDataGrid.RowCount; $i++ ) {
        ...
        $lModelName = $pDataGrid[2,$i].Value                          # column 2 has the Model name
        # get AddOns
        $lAddOnsRepoFile = $pRepositoryFolder+'\'+$addsoftware+'\'+$lModelId
        if (Test-Path $lAddOnsRepoFile) {
            # create an array string of softpaq names to add to the platform line
            [array]$lAddOns = Get-Content $lAddOnsRepoFile
            $lAddOnsString = "'$($lAddOns[0])'"
            for ($i=1; $i -lt $lAddOns.Count; $i++) { $lAddOnsString += ", '$($lAddOns[$i])'" }
            $lModelsList += "`n`t@{ ProdCode = '$($lModelId)'; Model = '$($lModelName)' ; AddOns = $($lAddOnsString) }"
        } else {
...
Function Browse_IndividualRepos {
    ...
    $lbrowse.Description = "Select an HPIA Folder as Head Repository"
    ...
    if ( $lbrowse.ShowDialog() -eq "OK" ) {
        ...
        Update_INIModels $pDataGrid $lRepository $False  # $False means treat as head of individual repositories
...
Function Browse_CommonRepo {
    ...
    if ( $lbrowse.ShowDialog() -eq "OK" ) {
        ...
        Try {
            ...
            Read_CommonRepofilters $pDataGrid $lRepository $True
            Update_UIandINI $lRepository $True
            Update_INIModels $pDataGrid $lRepository $True  # $False means treat as head of individual repositories
...
Function Import_CommonRepo {
    ...
    Try {
        ...
        if ( $lProdFilters.count -eq 0 ) {
            ...
            [array]$lRepoPlatforms = $lProdFilters.platform | Get-Unique
            # next, add product to the grid to then populate with the filters
            for ( $i = 0 ; $i -lt $lRepoPlatforms.count ; $i++) {
            ...
            Read_CommonRepofilters $pDataGrid $pCurrentRepository $True
            Update_UIandINI $pCurrentRepository $True
            Update_INIModels $pDataGrid $pCurrentRepository $True  # $False means treat as head of individual repositories
...
Function Manage_ID_File {
    ...
    if ( $Script:v_CommonRepo ) {
        $lProdIDFile = $v_Root_CommonRepoFolder+'\'+$AddSoftware+'\'+$lSysID
    } else {
...
Function CreateForm {
    ...
    $CommonRadioButton.Add_Click( {
        Import_CommonRepo $dataGridView $CommonPathTextField.Text 
    } ) # $CommonRadioButton.Add_Click()
    ...
    (Get-Content $IniFIleFullPath) | Foreach-Object { 
            if ($_ -match $find) { 
                if ( $_ -match '\$true' ) { 
                    $lCommonPath = $Script:v_Root_CommonRepoFolder
                    if ( ([string]::IsNullOrEmpty($lCommonPath)) -or `
                         (-not (Test-Path $lCommonPath)) ) {
                        Write-Host "Common Repository Path in INI file not found: $($lCommonPath) - Will Create" -ForegroundColor Red
                        Init_Repository $lCommonPath $True
                    }                    
                    $CommonRadioButton.Checked = $true                          # set the visual default from the INI setting
                    $CommonPathTextField.BackColor = $BackgroundColor
                } else { 
                    $lIndividualPath = $Script:v_Root_IndividualRepoFolder
    ...
    $buttonSync.add_click( {
        ...
        if ( $Script:v_OSVER -in $Script:v_OSVALID ) {
            ...
            if ( $Script:v_CommonRepo ) {
                Sync_Common_Repository $dataGridView $Script:v_Root_CommonRepoFolder $lCheckedListArray
            } else {
