...
Function Get_HPServerIP {
    ...
    start-job -Name $pJobName -scriptblock { 
        ...
        foreach ($connection in (Get-NetTCPConnection -state established -appliedsetting internet)) {
            if ( $connection.RemoteAddress -match '15.' ) {
        ...
        ForEach-Object {
            if ( $_.ProdCode -match $pProdCode ) {                              # let's match the Model to download for
                ...
                ForEach ( $Softpaq in $_.SqName ) {
                    Get_HPServerIP 'get-softpaqlist' $HPServerIPFile 2
...
Function Sync_and_Cleanup_Repository {
    ...
    if ( Test-Path $pFolder ) {
        ...
        # but first let's start a look at what connections (port 443) are being used, in a couple of seconds
...
#    }
    ...
    $LogPathLabel.Text = "Log File"
    $LogPathLabel.Location = New-Object System.Drawing.Point(($LeftOffset+380),($FormHeight-400))    # (from left, from top)
