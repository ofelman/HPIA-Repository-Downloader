...
[CmdletBinding()]
    ...
    if ( $Script:SingleRepo ) {
        ...
        CMTraceLog -Message  "... Updating a single repository $($lMainRepo)" -Type $TypeNorm
    } else {
    ...
    for ( $i = 0; $i -lt $pModelsList.RowCount; $i++ ) {
        ...
        if ( $i -in $pCheckedItemsList ) {
            ...
            if ( $debugMode ) { CMTraceLog -Message  "... removing filters for: $($lModelName)/$($lModelId)" -Type $TypeWarn }
...
Function CreateForm {
    ...
    $ClearTextBox.Location = New-Object System.Drawing.Point($LeftOffset,($FormHeight-45))    # (from left, from top)
