...
$ScriptVersion = "1.80 (July-27-2021)"
...
$GridChanged = $False                                # new for 1.80, to limit grid refreshes
...
<#
    Function init_repository
        ...
        Args:
            ...
            $pInitRepository: if $true, initialize repository, 
                $false: do not init (used as head of individual repository folders)
#>
Function init_repository {
    [CmdletBinding()]
	param( $pRepoFolder,
    ...
    $HPModelsTable  | ForEach-Object {
        if ( ($_.ProdCode -match $pProdCode) -and $pINISWSelected ) {                      # let's match the Model to download for
            ...
            ForEach ( $Softpaq in $_.AddOns ) {
                Try {
                    Get_HPServerIP 'get-softpaqlist' $HPServerIPFile 2      # check connections with 2 secs delay
                    $lListret = (Get-SoftpaqList -platform $pProdCode -os $OS -osver $v_OSVER -characteristic SSM) 6>&1 
...
Function Sync_Repos {
    ...
    if ( $Script:v_CommonRepo ) {
        ...
        if ( !(Test-Path "$($Script:v_Root_CommonRepository)\.Repository") ) {
            CMTraceLog -Message  "... Common Repository Folder selected, not initialized" -Type $TypeNorm
            return
        } 
        set-location $Script:v_Root_CommonRepository                 
        Sync_and_Cleanup_Repository $Script:v_Root_CommonRepository  
...
[CmdletBinding()]
    ...
    $lMainRepo =  $Script:v_Root_CommonRepository   
    ...
    for ( $item = 0; $item -lt $pModelsList.RowCount; $item++ ) {
        ...
        # if model entry row is checked, we need to create a repository, but ONLY if $v_CommonRepo = $false
        ...
        if ( $item -in $pCheckedItemsList ) {
            ...
            #--------------------------------------------------------------------------------
            download_softpaqs_by_name  $lMainRepo $lModelID $pModelsList.Rows[$item].Cells['AddOns'].Value
...
#=====================================================================================
<#
    Function Read_CommonRepofilters
...
Function Read_CommonRepofilters {
    ...
    if ( $DebugMode ) { CMTraceLog -Message '> Read_CommonRepofilters' -Type $TypeNorm }
    ...
    if ( !(Test-Path "$($pCommonFolder)\.Repository") ) {
        ...
        if ( $DebugMode ) { CMTraceLog -Message '< Read_CommonRepofilters - Done' -Type $TypeWarn }
        return
    } 

    if ( $pRefreshGrid ) {
        CMTraceLog -Message "Refreshing Grid from Common Repository: ''$pCommonFolder''" -Type $TypeNorm
        clear_datagrid $pDataGrid
    } else {
    ...
    foreach ( $filter in $lProdFilters ) {
        ...
        for ( $i = 0; $i -lt $pDataGrid.RowCount; $i++ ) {
            if ( $filter.platform -eq $pDataGrid[1,$i].value) {
                if ( $pRefreshGrid ) { 
                    # we matched the row/SysId with the Filter Platform IF, 
                    ...
                    # ... and last field/column is the Repository Path
                    $pDataGrid[($pDataGrid.ColumnCount-1),$i].Value = $pCommonFolder 
                    
                    # if we are maintaining AddOns Softpaqs, show it in the checkbox
                    $lAddOnRepoFile = $pCommonFolder+'\'+$addsoftware+'\'+$filter.platform   
                    if ( Test-Path $lAddOnRepoFile ) { 
                        $pDataGrid.Rows[$i].Cells['AddOns'].Value = $True
                        $lMsg = "...Additioanl Softpaqs Enabled for Platform ''$($filter.Platform)'':"+$Script:HPModelsTable[$i].AddOns
                        CMTraceLog -Message $lMsg -Type $TypeWarn 
                    }
                } else {
                    $lMsg = "... Platform $($filter.platform) ... $($filter.operatingSystem) $($filter.category) $($filter.characteristic)"
                    CMTraceLog -Message $lMsg -Type $TypeWarn
...
#=====================================================================================
<#
    Function Read_IndividualRepofilters
...
Function Read_IndividualRepofilters {
    ...
    if ( $DebugMode ) { CMTraceLog -Message '> Read_IndividualRepofilters' -Type $TypeNorm }
    ...
    for ( $i = 0; $i -lt $pDataGrid.RowCount; $i++ ) {
        ...
        $lTempRepoFolder = "$($Script:v_Root_IndividualRepoFolder )\$($lModelName)"                               # this is the repo folder for this model
        # move to location of Repository to use CMSL repo commands
...
Function get_filters {
    ...
    if ( $Script:v_CommonRepo ) {
        if ( -not ([string]::IsNullOrWhiteSpace($Script:v_Root_CommonRepository)) ) {
            $lPathIsValid = $True
            Read_CommonRepofilters $pDataGrid $Script:v_Root_CommonRepository $pRefreshGrid
        }
    } else {
        if ( -not ([string]::IsNullOrWhiteSpace($Script:v_Root_IndividualRepoFolder)) ) {
            $lPathIsValid = $True
            Read_IndividualRepofilters $pDataGrid $Script:v_Root_IndividualRepoFolder $pRefreshGrid 
        }
    } # else if ( $Script:v_CommonRepo )
...
#=====================================================================================
<#
    Function Update_HPModelsAndPath
...
Function Update_HPModelsAndPath {
    ...
    if ( $DebugMode ) { CMTraceLog -Message '> Update_HPModelsAndPath' -Type $TypeNorm }

    $lMsg = "Do you want to make this Models list the new default?"

    $lResponse = [System.Windows.MessageBox]::Show($lMsg,"Update INI File",4)    # 1 = "OKCancel" ; 4 = "YesNo"
    if ( $lResponse -eq 'Yes' ) {
        # -------------------------------------------------------------------
        ...
        for ( $i = 0; $i -lt $pDataGrid.RowCount; $i++ ) {
            $lModelId = $pDataGrid[1,$i].Value                            # column 1 has the Model/Prod ID
            $lModelName = $pDataGrid[2,$i].Value                          # column 2 has the Model name
            $lModelsList += "`n`t@{ ProdCode = '$($lModelId)'; Model = '$($lModelName)' }"
        } # for ()
        ...
        if ( Test-Path $lModelsListINIFile ) {
            ...
            # ----------------------------------------------------------------------------
            # next, list the new repository folder in the GUI's field and mod the INI file
            # ----------------------------------------------------------------------------
            if ( $pCommonRepoFlag ) {
                $CommonRadioButton.Checked = $true                          # set the visual default from the INI setting
                $SharedRadioButton.Checked = $false 
                $CommonPathTextField.Text = "$pRepositoryFolder"
                $CommonPathTextField.BackColor = $BackgroundColor
                $SharePathTextField.BackColor = ""

                $lfindValue = "^[\$]v_Root_CommonRepository"
                $lreplaceValue = "`$v_Root_CommonRepository = ""$pRepositoryFolder"""
                Mod_INISetting $IniFIleFullPath $lfindValue $lreplaceValue ">..$($IniFile) - changing setting: ''$($lreplaceValue)''"

                # ... and also mod the INI file to default to: 'v_CommonRepo = $True' or '$False'
                $lfindValue = "^[\$]v_CommonRepo"
                $lreplaceValue = "`$v_CommonRepo = `$$true"
                Mod_INISetting $IniFIleFullPath $lfindValue $lreplaceValue ">.. $($IniFile) - changing setting: ''$($lreplaceValue)''"
            } else {
                $CommonRadioButton.Checked = $False                          # set the visual default from the INI setting
                $SharedRadioButton.Checked = $True 
                $SharePathTextField.Text = "$pRepositoryFolder"
                $SharePathTextField.BackColor = $BackgroundColor
                $CommonPathTextField.BackColor = ""

                $lfindValue = "^[\$]v_Root_IndividualRepoFolder"
                $lreplaceValue = "`$v_Root_IndividualRepoFolder = ""$pRepositoryFolder"""
                Mod_INISetting $IniFIleFullPath $lfindValue $lreplaceValue ">..$($IniFile) - changing setting: ''$($lreplaceValue)''"

                # ... and also mod the INI file to default to: 'v_CommonRepo = $True' or '$False'
                $lfindValue = "^[\$]v_CommonRepo"
                $lreplaceValue = "`$v_CommonRepo = `$$False"
                Mod_INISetting $IniFIleFullPath $lfindValue $lreplaceValue ">.. $($IniFile) - changing setting: ''$($lreplaceValue)''"
...
<#
    Function Empty_Grid
        removes (clears out) all Model entries (rows) from the grid
#>
Function Empty_Grid {
    [CmdletBinding()]
	param( $pGrid )
...
Function Import_IndividualRepos {
    ...
    $lbrowse.SelectedPath = $Script:v_Root_IndividualRepoFolder         # start with the repo listed in the INI file
    $lbrowse.Description = "Select an HPIA Repository"
    $lbrowse.ShowNewFolderButton = $true

    # we want to re-read the current Individual Repositories
    if ( $pImportCurrentOnly -or ($lbrowse.ShowDialog() -eq "OK") ) {
    
        $lRepository = $lbrowse.SelectedPath

        # first remove existing rows from UI grid, starting from last one
        ...
        foreach ( $lFolder in $lDirectories ) {
            ...
            Try {
                $lRepoInfo = Get-RepositoryInfo
                $lFolderIsRepo = $true               # the folder is a proper HPIA repository
                CMTraceLog -Message  "... Repository Found ($($lRepoFolder))" -Type $TypeNorm

                CMTraceLog -Message  "... reading repository filters" -Type $TypeNorm
                $lProdFilters = $lRepoInfo.Filters
                # ... and parse to find tge platform SysID in the repository
                [array]$lRepoPlatforms = $lProdFilters.platform
                $row = @( $true, $lRepoPlatforms, $lFolder )
                [void]$pDataGrid.Rows.Add($row)
            } Catch {
                CMTraceLog -Message  "... NOT a Repository: ($($lRepoFolder))" -Type $TypeNorm
            }
        } # foreach ( $lFolder in $lDirectories )

        # now add a filter for every item in the DataGrid
        CMTraceLog -Message  "... Getting Filters from repositories"
        Read_IndividualRepofilters $pDataGrid $lRepository $True

        Update_HPModelsAndPath $pDataGrid $lRepository $False  # $False means treat as head of individual repositories
      
        $Script:v_Root_IndividualRepoFolder = $lRepository       # make the new repo be the new global variable                        
...
<#
    Function Import_CommonRepo
        Populate list of Platforms from an existing (common) Repository
        update INI file about imported repository if user agrees
        Paramaters: $pDataGrid = pointer to model grid in UI
                    $pImportCurrentOnly = $true: to re-import common/shared repository 
                                          $false: is to browse for existing or create new repository
        returns: object w/Folder picked for repository, or $null if user cancelled
                Folder return value contains True/Fail entry [0] + folder name [1]
#>
Function Import_CommonRepo {
    [CmdletBinding()]
	param( $pDataGrid, $pImportCurrentOnly )                                
    ...
    $lbrowse = New-Object System.Windows.Forms.FolderBrowserDialog
    $lbrowse.SelectedPath = $Script:v_Root_CommonRepository        # start with the repo listed in the INI file
    $lbrowse.Description = "Select an HPIA Repository"
    $lbrowse.ShowNewFolderButton = $true
    ...
    if ( $pImportCurrentOnly -or ($lbrowse.ShowDialog() -eq "OK") ) {
        ...
        Try {
            $lRepoInfo = Get-RepositoryInfo
            $Script:v_Root_CommonRepository = $lRepository  # make the new repo be the new script variable
            CMTraceLog -Message  "... HPIA Repository Found ($($lRepository))" -Type $TypeNorm
            # first remove existing rows from GUI grid, starting from last one
            CMTraceLog -Message  "... clearing Model list" -Type $TypeNorm
            Empty_Grid $pDataGrid
            CMTraceLog -Message  "... reading repo filters information" -Type $TypeNorm
            # now, obtain the CMSL filters from the repo
            $lProdFilters = $lRepoInfo.Filters
            # ... get the list of platform SysIDs in the repository
            [array]$lRepoPlatforms = $lProdFilters.platform | Get-Unique

            # next, add product rows to the grid to populate with the filters
            ...
            # Finally, populate the filters from the repository
            CMTraceLog -Message  "... Reading Filters from repository"
            Get_Filters $pDataGrid $True
                
            CMTraceLog -Message  "... updating INI file with new models and path" -Type $TypeNorm
            # ... create a Models list and modify the INI.ps1 config file w/ imported models
            Update_HPModelsAndPath $pDataGrid $lRepository $True  # $True means treat as single/common repo
            
            $lMsg = "NOTE: the models listed may not match your product as some models share Platform ID"
            CMTraceLog -Message $lMsg -Type $TypeWarn
            ...
            $lMsg = "Folder $($lRepository) is not set up as an HPIA repository!!`nDo you want to initialize it?`
NOTE: the Models list will remain so next Sync will populate from the listed models, as selected"
            $lRes = [System.Windows.MessageBox]::Show($lMsg,"Folder: $($lRepository)",1)    # 1 = "OKCancel" ; 4 = "YesNo"
            if ( $lRes -eq 'Ok' ) {
                init_repository $lRepository $true
                clear_datagrid $pDataGrid
                $Script:v_Root_CommonRepository = $lRepository  # make the new repo be the new script variable
                CMTraceLog -Message  "... Folder initialized as Repository" -Type $TypeNorm
                # need to set new repository default in INI file
                Update_HPModelsAndPath $pDataGrid $lRepository $True  # $True means treat as single/common repo
            } # if ( $lRes -eq 'Ok' )
        } # Catch
...
#=====================================================================================
<#
    Function Populate_Rows_from_HPModelsTable
...
} # Populate_Rows_from_HPModelsTable

#=====================================================================================
<#
    Function Set_Repo_Defaults
    Sets the v_CommonRepo flag and mods the path colors of the path fields
    ... as well as restting the INI file variable
#>
Function Set_Repo_Defaults {
    [CmdletBinding()]
	param( $pCommonFlag,                   # set $True if $v_CommonRepo = $True
        $pNewPath )                        # set the appropriate global folder name to this
    ...
    if ( $pCommonFlag ) {
        ...
        $CommonPathTextField.BackColor = $BackgroundColor
        $SharePathTextField.BackColor = ""
        $Script:v_Root_CommonRepository = $pNewPath
    } else {
    ...
    $find = "^[\$]v_CommonRepo"
    $replace = "`$v_CommonRepo = `$$Script:v_CommonRepo"  # set up the replacing string to either $false or $true from ini file

    Mod_INISetting $IniFIleFullPath $find $replace "$($Script:IniFile) - changing setting: ''$($replace)''"

} # Function Function Set_Repo_Defaults

#=====================================================================================
<#
    Function Manage_ID_File
    Parameters
        $pCreateFile
        $pGridList
        $pRow
...
Function Manage_ID_File {
    [CmdletBinding()]
	param( $pCreateFile,                    # $True = create file, $False = remove file
            $pGridList,
            $pRow )                         # 4-digit hex-code to use as the file name

    $lSysID = $pGridList.Rows[$pRow].Cells['SysID'].Value
    $lModelName = $pGridList.Rows[$pRow].Cells['Model'].Value

    if ( $Script:v_CommonRepo ) {
        $lProdIDFile = $v_Root_CommonRepository+'\'+$AddSoftware+'\'+$lSysID
    } else {
        $lProdIDFile = $v_Root_IndividualRepoFolder+'\'+$lModelName+'\'+$AddSoftware+'\'+$lSysID
    }

    if ( $pCreateFile ) {
        $lText = "... Enabling download of AddOns Softpaqs for Platform: $lSysID"
        if ( -not (Test-Path $lProdIDFile ) ) { New-Item $lProdIDFile }
    } else {
        $lText = "... Disabling download of AddOns Softpaqs for Platform: $lSysID"
        Remove-Item -Path $lProdIDFile -ErrorAction silentlycontinue
    } # else if ( $pCreateFile )

    CMTraceLog -Message $lText -Type $TypeNorm
...
Function CreateForm {
    ...
    $ShowHelp={
        ...
        Switch ($this.name) {
            ...
            "Individual Path" {$tip = "Path to Head of Individual platform repositories"}
    ...
    $OSVERComboBox.Location  = New-Object System.Drawing.Point(($LeftOffset+70), ($TopOffset))
    ...
    $keepFiltersCheckbox.Location = New-Object System.Drawing.Point(($LeftOffset+140),($TopOffset+2))   # (from left, from top)
    ...
    $continueOn404Checkbox.Location = New-Object System.Drawing.Point(($LeftOffset+140),($TopOffset+22))   # (from left, from top)
    ...
    $SharedRadioButton.Add_Click( {
            if ( -not ([string]::IsNullOrEmpty($Script:v_Root_IndividualRepoFolder)) ) {
                Set_Repo_Defaults $false $Script:v_Root_IndividualRepoFolder
                Import_IndividualRepos $dataGridView $True             # $True: browse current listed folder
            } # if ( -not ([string]::IsNullOrEmpty($Script:v_Root_IndividualRepoFolder)) )
        }
    ) # $SharedRadioButton.Add_Click()
    ...
    $SharePathTextField.Text = "$Script:v_Root_IndividualRepoFolder"
    ...
    $SharePathTextField.Name = "Individual Path"
    ...
    $sharedBrowse_Click = {
        $lnewRepoFolder = Import_IndividualRepos $dataGridView $false # $false: Browse/Import new folder
        if ( $lnewRepoFolder -ne $null ) {
            $v_CommonRepo = $False
            $Script:v_Root_IndividualRepoFolder = $lnewRepoFolder
            Set_Repo_Defaults $false $Script:v_Root_IndividualRepoFolder
            
            $SharePathTextField.Text = $lnewRepoFolder      # [0] returns $True or $False
            $find = "^[\$]v_Root_IndividualRepoFolder"
            $replace = "`$v_Root_IndividualRepoFolder = ""$($lnewRepoFolder)"""     # set up the replacing string to either $false or $true from ini file
            Mod_INISetting $IniFIleFullPath $find $replace "$($IniFile) - changing setting: ''$($replace)''"
        } # if ( $lnewRepoFolder -ne $null )
    } # $sharedBrowse_Click
    ...
    $CommonRadioButton.Add_Click( {
        Set_Repo_Defaults $True $Script:v_Root_CommonRepository
        if ( ([System.Windows.MessageBox]::Show("Do you want to restore the Models list from the INI file?","Refresh Grid",4)) -eq 'Yes' ) {  # 1 = "OKCancel" ; 4 = "YesNo"
            Empty_Grid $dataGridView
            Populate_Rows_from_HPModelsTable $dataGridView $Script:HPModelsTable
            Get_Filters $dataGridView $True 
        } else {
            Import_CommonRepo $dataGridView $True                 # $True = Import current repository folder
        }
    ...
    $CommonPathTextField.Text = "$Script:v_Root_CommonRepository"
    ...
    $commonBrowse_Click = {
        $lnewRepoFolder = Import_CommonRepo $dataGridView $False # $true: (re)-initialize as HPIA repo folder
        if ( $lnewRepoFolder -ne $null ) {
            Set_Repo_Defaults $True $Script:v_Root_CommonRepository
        }
    } # $commonBrowse_Click
    ...
    # Get default selection setting ($v_CommonRepo) for radio button from .INI file

    $find = "^[\$]v_CommonRepo"
    (Get-Content $IniFIleFullPath) | Foreach-Object { 
            if ($_ -match $find) { 
                if ( $_ -match '\$true' ) { 
                    if ( ([string]::IsNullOrEmpty($Script:v_Root_CommonRepository)) -or `
                         (-not (Test-Path $Script:v_Root_CommonRepository)) ) {
                        Write-Host "Common Path in INI file not found: $($Script:v_Root_CommonRepository)" -ForegroundColor Red
                        exit
                    }                    
                    $CommonRadioButton.Checked = $true                          # set the visual default from the INI setting
                    $CommonPathTextField.BackColor = $BackgroundColor
                } else { 
                    if ( ([string]::IsNullOrEmpty($Script:v_Root_IndividualRepoFolder)) -or `
                         (-not (Test-Path $Script:v_Root_IndividualRepoFolder)) ) {
                        Write-Host "Individual Path in INI file not found: $($Script:v_Root_IndividualRepoFolder)" -ForegroundColor Red
                        exit
                    }
                    $SharedRadioButton.Checked = $true 
                    $SharePathTextField.BackColor = $BackgroundColor
    ...
    $CellOnClick = {
        ...
        if ( $dataGridView.rows[$row].Cells[$column].Value.GetType() -eq [Boolean] ) {
            ...
            #            $datagridview.Rows[$row].Cells['AddOns'].State | out-host
            ...
            switch ( $column ) {
                0 {                                                               # need to reset all categories
                    if ( $CellNewState ) {
                        ...
                        $dataGridView.Rows[$row].Cells['BIOS'].Value = $true
                        $dataGridView.Rows[$row].Cells['Firmware'].Value = $true
                        $dataGridView.Rows[$row].Cells['Software'].Value = $true
                        $dataGridView.Rows[$row].Cells['AddOns'].Value = $true
                    } else {                               
                        $datagridview.Rows[$row].Cells['AddOns'].Value = $false   # reset the 'All' column
                ...
                Default {   
                    ...
                    if ( $datagridview.Rows[$row].Cells['AddOns'].State -eq 'Selected' ) {
                        Manage_ID_File $CellNewState $datagridview $row
                        #Manage_ID_File $CellNewState $datagridview.Rows[$row].Cells['SysID'].Value
                    }
                    if ( -not $CellNewState ) {
    ...
    $GetModelsFromINIButton.Text = 'Get Models from INI'
    ...
    $RefreshGridButton.Location = New-Object System.Drawing.Point(($LeftOffset+100),($FormHeight-447))    # (from left, from top)
    ...
    $LogPathLabel.Location = New-Object System.Drawing.Point(($LeftOffset+325),($FormHeight-442))    # (from left, from top)
    ...
    $LogPathField.location = New-Object System.Drawing.Point(($LeftOffset+355),($FormHeight-445)) # (from left, from top)
...
<#
    #----------------------------------------------------------------------------------
    # Create Save to INI button - "TBD"
    #----------------------------------------------------------------------------------
    if ( $DebugMode ) { Write-Host 'creating Save to INI button' }
    $ButtonSave = New-Object System.Windows.Forms.Button
    $ButtonSave.Width = 60
    $ButtonSave.Height = 35
    $ButtonSave.Text = 'Save to INI'
    $ButtonSave.Location = New-Object System.Drawing.Point(($FormWidth-220),($FormHeight-447))

    #$CM_form.Controls.AddRange(@($ButtonSave))
#>
    #----------------------------------------------------------------------------------
    # Create Sync button
    #----------------------------------------------------------------------------------
    ...
    $buttonSync.Location = New-Object System.Drawing.Point(($FormWidth-120),($FormHeight-447))
...
######################################################################################################
    ...
    get_filters $dataGridView $True           # $True = refresh grid from repository
    if ( $DebugMode ) { Write-Host 'checking if selected OS Version supported by each platform' }
    Check_PlatformsOSVersion $dataGridView $OSVERComboBox.SelectedItem
