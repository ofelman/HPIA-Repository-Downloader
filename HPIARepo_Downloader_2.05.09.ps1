...
$ScriptVersion = "2.05.09 (Jul 28, 2026)"
...
if ( $help ) {
    ...
    "`nHPIARepo_Downloader_2.05.09.ps1 [[-Sync] [-ListFilters] [-inifile .\<filepath>\HPIARepo_ini.ps1] [-RepoStyle common|individual] [-products '80D4,8549,8470'] [-NoIniSw] [-ShowActivityLog]]`n"
    ...
    "`nExample: HPIARepo_Downloader_2.05.09.ps1 .\<path>\HPIARepo_ini.ps1 Common '80D4,8549,8470'`n"    
...
# -UseBasicParsing is needed for PowerShell v3 and v4 to avoid potential script run from returned page
'My Public IP: '+(Invoke-WebRequest ifconfig.me/ip -UseBasicParsing).Content.Trim() | Out-Host
...
function Load_HPCMSLModule {
    ...
    if (Get-Module | Where-Object {$_.Name -eq $m}) {
        if ( $v_DebugMode ) { write-host "Module $m is already imported." ; CMTraceLog -Message "Module already imported." -Type $TypeSuccess }
    } else {
        ...
        if (Get-Module -ListAvailable | Where-Object {$_.Name -eq $m}) {
            ...
            if ( $v_DebugMode ) { CMTraceLog -Message "Done" -Type $TypeSuccess }
        } else {

            # If module is not imported, not available on disk, but is in online gallery then install and import
            if (Find-Module -Name $m | Where-Object {$_.Name -eq $m}) {
                if ( $v_DebugMode ) { write-host "Upgrading NuGet and updating PowerShellGet first." ; CMTraceLog -Message "Upgrading NuGet and updating PowerShellGet first." -Type $TypeSuccess}
                ...
                if ( $v_DebugMode ) { write-host "Installing and Importing Module $m." ; CMTraceLog -Message "Installing and Importing Module $m." -Type $TypeSuccess}
                ...
                exit 1
            } # else if (Find-Module -Name $m | Where-Object {$_.Name -eq $m})
    ...
    CMTraceLog -Message "Using CMSL version: $($hpcmsl.Version)" -Type $TypeError    
...
<#
    Function Init-Repository
        ...
        Args:
            ...
            $pInitialize:   $true, initialize repository, 
...
Function Init-Repository {
	...
	param( $pRepoFolder,
            $pInitialize )
    if ( $v_DebugMode ) { CMTraceLog -Message "> Init-Repository()" -Type $TypeNorm }
    ...
    switch ( $pInitialize ) {
        $true {
            ...
            Try {
                # Attempt to get repository info to check if it's already initialized, if NOT initialized, Get-RepositoryInfo will throw an error, which we catch to initialize the repository
                $ir_RepoInfoJob = Get-RepositoryInfo -ErrorAction Stop
                ...
                if ($v_DebugMode) { CMTraceLog -Message "... initializing repository" -Type $TypeNorm }
                ...
                #    Set-Location $using:pRepoFolder
                #    Import-Module HPCMSL
                #    Initialize-Repository
                #    Set-RepositoryConfiguration -setting OfflineCacheMode -cachevalue Enable
                #    Set-RepositoryConfiguration -setting RepositoryReport -Format csv
                #} | Wait-Job | Receive-Job
                if ( $v_DebugMode ) { CMTraceLog -Message  "... repository initialized and configured for HP Image Assistant"  -Type $TypeNorm }
...
Function Remove-SoftpaqFiles {
    param( $pSoftpaqID, 
            $pAddOnFolderPath )

    $rs_SoftpaqPath = "$pAddOnFolderPath\$($pSoftpaqID)"
    Try {
        $rs_exe = $rs_SoftpaqPath+'.exe'
        if ( $v_DebugMode ) { CMTraceLog -Message "`tremoving: $($rs_exe) and associated cva, html files" -Type $TypeNorm }
        Remove-Item $rs_exe -Force -ErrorAction SilentlyContinue
        $rs_cva = $rs_SoftpaqPath+'.cva'
        Remove-Item $rs_cva -Force -ErrorAction SilentlyContinue
        $rs_html = $rs_SoftpaqPath+'.html'
        Remove-Item $rs_html -Force -ErrorAction SilentlyContinue
    } Catch {
        if ( $v_DebugMode ) { CMTraceLog -Message "`tFailed to remove superseded Softpaq files: $($_)" -Type $TypeError }
    } 
    
} # Function Remove-SoftpaqFiles()
Function Remove-SupersededSoftpaqs {
    param( $pSoftpaq, 
            $pAddOnFolderPath )

    $rs_currLocation = Get-Location

    $rs_paths = @($pAddOnFolderPath, (Split-Path $pAddOnFolderPath -Parent) )

    if ( $v_DebugMode ) { CMTraceLog -Message "`tremoving superseded softpaqs for: $($pSoftpaq.id)/$($pSoftpaq.Name)" -Type $TypeNorm }

    $rs_paths | ForEach-Object {
        $rs_SupersededSoftpaqExe = Find-SupersedeDownloaded "$_\$($pSoftpaq.id).exe"
        while ( $rs_SupersededSoftpaqExe ) {
            if ( $v_DebugMode ) { CMTraceLog -Message "`tFound superseded Softpaq ($($pSoftpaq.id)) - will remove" -Type $TypeWarn }
            $rs_SoftpaqID = [System.IO.Path]::GetFileNameWithoutExtension($rs_SupersededSoftpaqExe)
            Remove-SoftpaqFiles -pSoftpaqID $rs_SoftpaqID -pAddOnFolderPath $_
            # find next superseded Softpaq, if any
            $rs_SupersededSoftpaqExe = Find-SupersedeDownloaded "$_\$($rs_SoftpaqID).exe"
        } # while ( $rs_SupersededSoftpaqexe )
    }
    
    set-location $rs_currLocation
...
<#
    Function Get-SoftpaqFiles
        ...
        In addition, after downloading is confirmed, it removes all previously downloaded superseded Softpaq files
        Args:
...
Function Get-SoftpaqFiles {
    ...
    # start downloading the Softpaq EXE file

    $ds_SoftpaqNamePath = $pAddOnFolderPath+'\'+$pSoftpaq.id

    $ds_SoftpaqExePath = $ds_SoftpaqNamePath+'.exe'
    
    CMTraceLog -Message "... downloading AddOn Softpaq$($pSoftpaq)"
    $ds_downloadSucceeded = $true   # assume download completes or already downloaded
    if ( Test-Path $ds_SoftpaqExePath ) {
        if ( $v_DebugMode ) { CMTraceLog -Message "`t$($pSoftpaq.id).exe already downloaded. $($pSoftpaq.Name)" -Type $TypeWarn }
    } else {                                        
        if ( $v_DebugMode ) { CMTraceLog -Message "`tdownloading $($pSoftpaq.id)/$($pSoftpaq.Name)" -Type $TypeNoNewline }
        Try {
            $gs_softpaqId = $pSoftpaq.Id
            Start-Job -ScriptBlock { Set-Location $using:pAddOnFolderPath; Import-Module HPCMSL; Get-HPSoftpaq $using:gs_softpaqId } | Wait-Job | Receive-Job
            if ( $v_DebugMode ) { CMTraceLog -Message "... done" -Type $TypeNorm }
        } Catch {
            if ( $v_DebugMode ) { CMTraceLog -Message "... failed to download: $($_)" -Type $TypeError }
            $ds_downloadSucceeded = $false
        } # Try/Catch block for Get-HPSoftpaq

    } # else if ( Test-Path $ds_SoftpaqExePath ) {

    # download associated CVA file

    if ( $v_DebugMode ) { CMTraceLog -Message "`tdownloading $($pSoftpaq.id).cva file" -Type $TypeNoNewline }
    Try {
        $gs_softpaqId2 = $pSoftpaq.Id
        Start-Job -ScriptBlock { Set-Location $using:pAddOnFolderPath; Import-Module HPCMSL; Get-HPSoftpaqMetadataFile $using:gs_softpaqId2 -Overwrite 'Yes' } | Wait-Job | Receive-Job
        if ( $v_DebugMode ) { CMTraceLog -Message "... done" -Type $TypeNorm }
    } Catch {
        if ( $v_DebugMode ) { CMTraceLog -Message "... failed to download: $($_)" -Type $TypeError }
    } # Try/Catch block for Get-HPSoftpaqMetadataFile
    
    # download corresponding HTML file

    if ( $v_DebugMode ) { CMTraceLog -Message "`tdownloading $($pSoftpaq.id) HTML file" -Type $TypeNoNewline }
    Try {
        $da_SoftpaqHtml = $ds_SoftpaqNamePath+'.html' # where to download to
        Invoke-WebRequest -UseBasicParsing -Uri $pSoftpaq.ReleaseNotes -OutFile "$da_SoftpaqHtml"
        if ( $v_DebugMode ) { CMTraceLog -Message "... done" -Type $TypeNorm }
    } Catch {
        if ( $v_DebugMode ) { CMTraceLog -Message "... failed to download" -Type $TypeError }
    }    
    
    # remove superseded Softpaqs only if the new Softpaq was successfully downloaded
    if ( $ds_downloadSucceeded) {
        Remove-SupersededSoftpaqs -pSoftpaq $pSoftpaq -pAddOnFolderPath $pAddOnFolderPath
    }
...
Function Find-SupersedeDownloaded {
    [CmdletBinding()] 
    param( $pAddOnFlagFileFullPath )
    
    $fd_superseded = $null

    if ( Test-Path $pAddOnFlagFileFullPath ) {
        # Get the folder containing the CVA file
        $fd_addOnFolder = Split-Path $pAddOnFlagFileFullPath -Parent

        # get the basename from the full path without extension
        $fd_supersededSoftpaqID = [System.IO.Path]::GetFileNameWithoutExtension($pAddOnFlagFileFullPath)

        # find the superseded softpaq number and create a path to it in the argument folder
        $fd_supersededSoftpaq = (Start-Job -ScriptBlock { Import-Module HPCMSL; (Get-HPSoftpaqMetadata $using:fd_supersededSoftpaqID).Softpaq.SupersededSoftpaqNumber } | Wait-Job | Receive-Job)
        $fd_supersededPath = "$fd_addOnFolder\$fd_supersededSoftpaq.exe"
        # check if the superseded Softpaq file exists in the AddOn folder
        if ( Test-Path $fd_supersededPath ) {
            $fd_superseded = $fd_supersededPath
        }
    } # if ( Test-Path $pAddOnFlagFileFullPath )

    return $fd_superseded

} # Function Find-SupersedeDownloaded()

Function Get-AddOnSoftpaqs {
[CmdletBinding()] 
    param( $pAddOnFlagFileFullPath )

    if ( -not (Test-Path $pAddOnFlagFileFullPath) ) { return }

    $ga_ProdCode = Split-Path $pAddOnFlagFileFullPath -leaf
    $ga_AddSoftpaqsFolder = Split-Path $pAddOnFlagFileFullPath -Parent

    [array]$gs_AddOnsList = Get-Content $pAddOnFlagFileFullPath

    if ( $gs_AddOnsList.count -gt 0 ) {
        if ( $v_DebugMode ) { CMTraceLog -Message "... platform $($ga_ProdCode): checking for Softpaq AddOns flag file entries" }
        
        Try {
            $_ga_os = $Script:v_OS; $_ga_osver = $Script:v_OSVER
            $ga_SoftpaqList = Start-Job -ScriptBlock { Import-Module HPCMSL; Get-HPSoftpaqList -platform $using:ga_ProdCode -os $using:_ga_os -osver $using:_ga_osver -ReferenceUrl 'https://hpia.hpcloud.hp.com/ref/' -characteristic SSM } | Wait-Job | Receive-Job
            # check every reference file Softpaq for a match in the flg file list
            ForEach ( $iAddOnEntry in $gs_AddOnsList ) {
                $ga_EntryFoundFlag = $False                             # set the entry found flag to false
                if ( $v_DebugMode ) { CMTraceLog -Message "...   Checking AddOn Softpaq by name: $($iAddOnEntry)" -Type $TypeNorm }
                # check every Softpaq in the refrerence file list for a match with the flag file entry
                # ... if found, download the Softpaq
                ForEach ( $iSoftpaq in $ga_SoftpaqList ) {
                    # adding check to exclude drivers (v.2.05.03) ex. 'HP App' matches 'HP Application Enabling Software Driver'
                    if ( [string]$iSoftpaq.Name -match $iAddOnEntry -and ($iSoftpaq.Category -notmatch '^Driver') ) {
                        $ga_EntryFoundFlag = $True                        
                        Get-SoftpaqFiles $iSoftpaq $ga_AddSoftpaqsFolder
                    } # if ( [string]$iSoftpaq.Name -match $iAddOnEntry )
                } # ForEach ( $iSoftpaq in $ga_SoftpaqList )
                if ( -not $ga_EntryFoundFlag ) {
                    if ( $v_DebugMode ) { CMTraceLog -Message  "... '$($iAddOnEntry)': Softpaq not found for this platform and OS version"  -Type $TypeWarn }
                } # if ( -not $ga_EntryFoundFlag )
            } # ForEach ( $iAddOnEntry in $gs_AddOnsList )
        } Catch {
            if ( $v_DebugMode ) { CMTraceLog -Message "... $($ga_ProdCode): Error retrieving Reference file" -Type $TypeError }
        }                        
    } else {
        if ( $v_DebugMode ) { CMTraceLog -Message '    '$ga_ProdCode': Flag file found but empty ' }
    ...
    switch ( $pCommonRepoFlag ) {
        ...
        $true { 
            ...
            for ( $iRow = 0; $iRow -lt $pGrid.RowCount; $iRow++ ) {
                ...
                if ( $da_AddOnsFlag ) {                    
                    Get-AddOnSoftpaqs $da_ProdIDFlagFile 
        ...
        $false {    # Download AddOns for individual models ( $pCommonRepoFlag = $false )
            ...
            for ( $i = 0; $i -lt $pGrid.RowCount; $i++ ) {
                ...
                $da_SystemName = $pGrid[2,$i].Value                           # column 2 has the Model name
                $da_Foldername = split-path $pFolder -Leaf

                if ( $da_Foldername -match $da_SystemName ) {                   # we found the model, so get the Prod ID
                    $da_AddOnsSettingFlag = $pGrid.Rows[$i].Cells['AddOns'].Value   # column 7 has the AddOns checkmark
                    $da_ProdIDFlagFile = $da_AddSoftpaqsFolder+'\'+$da_SystemID
                    if ( $da_AddOnsSettingFlag ) {
                        Get-AddOnSoftpaqs $da_ProdIDFlagFile                   
                    } # if ( $da_AddOnsSettingFlag -and (Test-Path $da_ProdIDFlagFile) )                    
                } # if ( $da_Foldername -match $da_SystemName )
            } # for ( $i = 0; $i -lt $pGrid.RowCount; $i++ )
        } # false
...
Function Restore-AddOns {
    ...
    if ( $pAddSoftpaqs ) {
        if ( $v_DebugMode ) { CMTraceLog -Message "...    Restoring named softpaqs files to Repository" -Type $TypeNoNewline }
...
#=====================================================================================
<#
    Function Sync-andCleanupRepository
...
Function Sync-andCleanupRepository {
    ...
    CMTraceLog -Message  "    >> Sync-andCleanupRepository - '$pFolder' - please wait !!!" -Type $TypeNorm
    ...
    if ( Test-Path $pFolder ) {
        ...
        Try {
            if ( $v_DebugMode ) { CMTraceLog -Message  '...   calling CMSL Invoke-HPRepositorySync -- please wait !!' -Type $TypeNorm }
            Start-Job -ScriptBlock { Set-Location $using:pFolder; Import-Module HPCMSL; Invoke-HPRepositorySync } | Wait-Job | Receive-Job
            ...
            try {
                ...
                Get-ActivityLogEntries $pFolder  # get sync'd info from HPIA activity log file
                ...
                #--------------------------------------------------------------------------------
                if ( $v_DebugMode ) { CMTraceLog -Message  '...   calling CMSL invoke-HPRepositoryCleanup' -Type $TypeNorm }
                Invoke-HPRepositoryCleanup 6>&1

                if ( $v_DebugMode ) { CMTraceLog -Message  '...   > Get-AddOns()' -Type $TypeNorm }
                Get-AddOns $pGrid $pFolder $pCommonFlag
                if ( $v_DebugMode )  {CMTraceLog -Message  '...   > Restore-AddOns()' -Type $TypeNorm }
                Restore-AddOns $pFolder (-not $script:noIniSw)
                #--------------------------------------------------------------------------------
        ...
        CMTraceLog -Message  '    << Sync-andCleanupRepository - Done' -Type $TypeNorm
...
[CmdletBinding()]
    ...
    foreach ( $cat in $Script:v_FilterCategories ) {
        ...
        $umf_CatSettingFlag = $pGrid.Rows[$pRow].Cells[$cat].Value
        if ( $umf_CatSettingFlag ) {
            if ( $v_DebugMode )  { CMTraceLog -Message  "... adding filter: -Platform $pModelID -os $Script:v_OS:$Script:v_OSVER -category $($cat) -characteristic ssm" -Type $TypeNormal }
...
[CmdletBinding()]
    ...
    if ( $pCommonRepoFlag ) {
        ...
        Sync-andCleanupRepository $pGrid $Script:v_Root_CommonRepoFolder $pCommonRepoFlag
        ...
        if ( Test-Path $Script:v_Root_IndividualRepoFolder ) {
            ...
            foreach ( $iprodName in $sr_ProdFolders ) {
                ...
                Sync-andCleanupRepository $pGrid $sr_CurrentPath $pCommonRepoFlag
            } # foreach ( $iprodName in $sr_ProdFolders )
        } else {
...
#=====================================================================================
<#
    Function sync-individualRepositories
...
[CmdletBinding()]
	param( $pGrid,
            $pRepoHeadFolder,            
            $pNewModels )                               # $True = models added to list)
    
    CMTraceLog -Message "> sync-individualRepositories() - START" -Type $TypeNorm
    if ( $v_DebugMode ) { CMTraceLog -Message "... for OS version: $($Script:v_OS):$($Script:v_OSVER), at: $($pRepoHeadFolder)" -Type $TypeNorm }

    #--------------------------------------------------------------------------------
    # decide if we work on single repo or individual repos
    #--------------------------------------------------------------------------------     
    Init-Repository $pRepoHeadFolder $false             # make sure Main repo folder exists, do NOT make it a repository

    #--------------------------------------------------------------------------------
    if ( $v_DebugMode ) { CMTraceLog -Message "... stepping through selected models" -Type $TypeWarn }

    # go through every selected HP Model in the list, each has a repository folder
    for ( $i = 0; $i -lt $pGrid.RowCount; $i++ ) {
        $si_ModelId = $pGrid[1,$i].Value                    # grid column 1 has the Model/Prod ID
        $si_ModelName = $pGrid[2,$i].Value                  # grid column 2 has the Model name
        ...
        if ( $pGrid[0,$i].Value ) {
            ...
            if ( $v_DebugMode ) { CMTraceLog -Message "`n... Syincing platform: $si_ModelId : $si_ModelName -- path $si_RepoFolder" -Type $TypeDebug }
            
            Init-Repository $si_RepoFolder $true            # initialize the repository folder for this model, if it does not exist   
            
            Update-ModelFilters $pGrid $si_RepoFolder $si_ModelId $i       # update filters for the current model in the grid
            
            Sync-andCleanupRepository $pGrid $si_RepoFolder $False        # invoke sync and cleanup for this individual repository folder
    ...
    if ( $pNewModels ) { Update-INIModelsFromGrid $pGrid $pCommonRepoFolder }

    CMTraceLog -Message "< sync-individualRepositories() DONE" -Type $TypeSuccess

} # Function sync-individualRepositories

#=====================================================================================
<#
    Function Sync-CommonRepository
...
Function Sync-CommonRepository {
    [CmdletBinding()]
        param( $pGrid,                              # the UI grid of platforms in the form
    ...
    CMTraceLog -Message "> Sync-CommonRepository - START" -Type $TypeSuccess
    if ( $v_DebugMode ) { CMTraceLog -Message "... for OS version: $($Script:v_OSVER)" -Type $TypeDebug }

    Init-Repository $pRepoFolder $true              # make sure repo folder exists, or create it, initialize
    ...
    for ( $iRow = 0; $iRow -lt $pGrid.RowCount; $iRow++ ) { 
        if ( $pGrid[0,$iRow].Value -eq $false ) { $sc_UnselectedRows = $true ; break } 
    } # for ( $iRow = 0; $iRow -lt $pGrid.RowCount; $iRow++ )

    $sc_SyncConfirm = 'Yes'                         # default to 'Yes' to proceed with sync even if unselected rows found

    if ( $sc_UnselectedRows ) {
        $sc_SyncConfirm = AskUser 'There are unselected entries. OK to Sync and remove their content?' 4 # 4="YesNo"
    } # if ( $sc_UnselectedRows )

    if ( $sc_SyncConfirm -eq 'Yes') {

        #$sc_filters = Start-Job -ScriptBlock { Set-Location $using:pRepoFolder; Import-Module HPCMSL; (Get-RepositoryInfo).Filters } | Wait-Job | Receive-Job
        $sc_filters = (Get-RepositoryInfo).Filters

        # now proceed with the sync operation for the common repository, one row at a time
        for ( $iRow = $pGrid.RowCount-1; $iRow -ge 0; $iRow-- ) {

            $sc_ModelId = $pGrid[1,$iRow].Value                 # col 1 has the SysID
            $sc_ModelName = $pGrid[2,$iRow].Value               # col 2 has the Model name

            if ( -not $Script:v_KeepFilters ) {
                if ( $sc_filters.count -gt 0 ) { Remove-HPRepositoryFilter -platform $sc_ModelId -yes 6>&1 }
                if ( $v_DebugMode ) { CMTraceLog -Message "... removed existing filters for platform: $($sc_ModelId):$($sc_ModelName)" -Type $TypeWarn }
                Update-ModelFilters $pGrid $pRepoFolder $sc_ModelId $iRow # update filters for the current model in the grid
            } # if ( -not $Script:v_KeepFilters )

            if ( $pGrid[0,$iRow].Value -eq $false ) {
                Remove-SinglePlatform $pGrid $iRow $pRepoFolder $true
            } # if ( $pGrid[0,$iRow].Value -eq $false )
        
        } # for ( $iRow = 0; $iRow -lt $pGrid.RowCount; $iRow++ )

        # finally, after updating all models, we can proceed to sync and cleanup the common repository
        CMTraceLog -Message "... Syncing Common Repository: $pRepoFolder" -Type $TypeNoNewline
        Update-INIModelsFromGrid $pGrid $pRepoFolder    # update the INI file with the current models in the grid, if needed
        Sync-andCleanupRepository $pGrid $pRepoFolder $True # $True = sync a common repository

    } # if ( $sc_SyncConfirm -eq 'Yes')
    
    Set-Location -Path $sc_CurrentLoc

    CMTraceLog -Message "< Sync-CommonRepository DONE" -Type $TypeSuccess
...
Function clear-grid {
    ...
    for ( $row = 0 ; $row -lt $pGrid.RowCount ; $row++ ) {
        # for each row, clear each cell, except product ID and platform name
        for ( $col = 0 ; $col -lt $pGrid.ColumnCount ; $col++ ) {
            # clear all columns, except 1,2 - 'SysID' and 'System Name'
            if ( $col -in @(0,3,4,5,6,7,8) ) {
                $pGrid[$col,$row].value = $False                       # clear checkmarks
            } else {
                # clear the 'repository path' cell (last column)
                if ( $pGrid.columns[$col].Name -match 'Repository' ) {
                    $pGrid[$col,$row].value = ''                       # clear path text field
                }
            } # else if ( $col -in @(0,3,4,5,6,7,8) )
        } # for ( $col = 0 ; $col -lt $pGrid.ColumnCount ; $col++ )
    } # for ( $row = 0 ; $row -lt $pGrid.RowCount ; $row++ ) 
...
Function list-filters {
    ...
    if ( $Script:v_CommonRepo ) {
        ...
        set-location $Script:v_Root_CommonRepoFolder
        $_lf_commonPath = $Script:v_Root_CommonRepoFolder
        #$lf_ProdFilters = Start-Job -ScriptBlock { Set-Location $using:_lf_commonPath; Import-Module HPCMSL; (Get-RepositoryInfo).Filters } | Wait-Job | Receive-Job
        ...
        foreach ( $iprodName in $lf_ProdFolders ) {
            $_lf_indivPath = "$($Script:v_Root_IndividualRepoFolder)\$($iprodName.name)"
            set-location $_lf_indivPath

            #$lf_ProdFilters = Start-Job -ScriptBlock { Set-Location $using:_lf_indivPath; Import-Module HPCMSL; (Get-RepositoryInfo).Filters } | Wait-Job | Receive-Job
            $lf_ProdFilters = (Get-RepositoryInfo).Filters
            $lf_platforms = $lf_ProdFilters.platform | Get-Unique 
            $lf_characteristics = $lf_ProdFilters.characteristic | Get-Unique            
            $lf_OS = $lf_ProdFilters.operatingSystem | Get-Unique 
...
#=====================================================================================
<#
    Function Get-CommonRepofilters
...
Function Get-CommonRepofilters {
	...
	param( $pGrid,                                          # array of row lines that are checked
        $pCommonFolder )                                    # if $true, refresh grid,  $false, just list filters

    if ( $v_DebugMode ) { CMTraceLog -Message '... > Get-CommonRepofilters' -Type $TypeNorm }
    Try { 
        clear-grid $pGrid
        set-location $pCommonFolder

        $gc_ProdFilters = (Get-RepositoryInfo).Filters
        ...
        foreach ( $ifilter in $gc_ProdFilters ) {
            # check each row SysID against the assigned Filter Platform ID/SysId so we can match it up
            # ... and then set the checkmarks for each category in the filter
            for ( $iRow = 0; $iRow -lt $pGrid.RowCount; $iRow++ ) {

                $gc_SysID = $pGrid[1,$iRow].Value                               # column 1 has the Model/Prod ID
                if ( $ifilter.platform -eq $gc_SysID ) {                        # we matched the row/SysId with the Filter

                    if ( $v_DebugMode ) { CMTraceLog -Message "... found filter for platform: $($ifilter.platform)" -Type $TypeDebug }
                    $pGrid[0,$iRow].Value = $true                               # set the selection column - 1st column in the grid                   
                    foreach ( $icat in  ($ifilter.category.split(' ')) ) {      # Let's add each category in the filter to the Model in the GUI
                        $pGrid.Rows[$iRow].Cells[$icat].Value = $true
                    } # foreach ( $icat in  ($ifilter.category.split(' ')) )
                    
                    $pGrid[($pGrid.ColumnCount-1),$iRow].Value = $pCommonFolder # ... and the Repository Path (the last column)
                    
                    # if we are maintaining AddOns Softpaqs (SysID flag file has content), show it in AddOns checkbox
                    $gc_AddOnsFlagFile = $pCommonFolder+'\'+$MyAddSoftwareFolder+'\'+$ifilter.platform  # path to flag file for this model                    

                    if ( (Test-Path $gc_AddOnsFlagFile) -and (-not ($pGrid.Rows[$iRow].Cells['AddOns'].Value))) {
                        $pGrid.Rows[$iRow].Cells['AddOns'].Value = $True        # default: flag file exists and is not empty
                        [array]$gc_AddOns = Get-Content $gc_AddOnsFlagFile
                        if ( $gc_AddOns.count -gt 0 ) {
                            if ( $v_DebugMode ) { CMTraceLog -Message "...  Additional Softpaqs to be maintained for $($ifilter.platform): { $gc_AddOns }" -Type $TypeWarn }
                        } else {
                            $pGrid.Rows[$iRow].Cells['AddOns'].Value = $False   # the AddOns flag file is empty
                        } # else if ( $gc_AddOns.count -gt 0 )
                    } # if ( (Test-Path $gc_AddOnsFlagFile) -and (-not ($pGrid.Rows[$iRow].Cells['AddOns'].Value)) )

                } # if ( $ifilter.platform -eq $pGrid[1,$i].value )
        ...
        CMTraceLog -Message "... Error accessing Repository Filters: $_" -Type $TypeError
        if ( $v_DebugMode ) { CMTraceLog -Message '< Get-CommonRepofilters - Done' -Type $TypeError }
    } # Catch Try
...
#=====================================================================================
<#
    Function Get-IndividualRepofilters
...
Function Get-IndividualRepofilters {
    ...
    if ( $v_DebugMode ) { CMTraceLog -Message '   > Get-IndividualRepofilters() START' -Type $TypeNorm }
    ...
    for ( $iRow = 0; $iRow -lt $pGrid.RowCount; $iRow++ ) {
        ...
        # write-Host "... checking for repository for model: $($gi_ModelId):$($gi_ModelName)" -ForegroundColor Yellow
        ...
        if ( Test-Path $gi_PlatformRepoFolder ) {
            ...
            # write-Host "... adding value for repo path: $($gi_PlatformRepoFolder)" -ForegroundColor Yellow
            ...
            CMTraceLog -Message "Checking for AddOns flag file: $gi_AddOnRepoFile" -Type $TypeNorm
            ...
            if ( Test-Path $gi_AddOnRepoFile ) {
                ...
                if ( $gi_AddOnsList.count -gt 0 ) {
                    ...
                    if ( $v_DebugMode ) { CMTraceLog -Message "...  AddOns flag file is empty for '$gi_ModelName'" -Type $TypeWarn }
                } # else if ( $gi_AddOnsList.count -gt 0 )
            } # if ( Test-Path $gi_AddOnRepoFile )
            ...
            <# populate the grid with the filters for this model
                ...
                characteristic  : ssm
            #>
            #$gi_ProdFilters = Start-Job -ScriptBlock { Set-Location $using:gi_PlatformRepoFolder; Import-Module HPCMSL; (Get-RepositoryInfo).Filters } | Wait-Job | Receive-Job
    ...
    if ( $v_DebugMode ) { CMTraceLog -Message '   < Get-IndividualRepofilters() DONE' -Type $TypeNorm }
...
<#
    Function Empty-Grid
        empty the grid of all rows, effectively clearing it
        Parameters: $pGrid - the DataGridView object to be emptied
#>
Function Empty-Grid {
    [CmdletBinding()]
	param( $pGrid )
...
<#
    Function Get-RepoFolder
        Lets the user point to an existing repository folder or create a new one
        - This function is used to browse for a repository folder, either for individual or common repositories
        Paramaters: $pCurrentRepository
                    $pText
        returns: hash table with two keys: 
            - Return: "OK" or "Cancel" (dialog result)
            - SelectedPath: the path selected by the user, or empty string if cancelled
#>
Function Get-RepoFolder {
    [CmdletBinding()] param( $pCurrentRepository, $pAskText )
    # ask the user for the repository

    $gr_browse = New-Object System.Windows.Forms.FolderBrowserDialog
    $gr_browse.SelectedPath = $pCurrentRepository ; $gr_browse.Description = $pAskText
    $gr_browse.ShowNewFolderButton = $true

    $gr_return = @{}
    $gr_return.return = $gr_browse.ShowDialog()
    $gr_return.SelectedPath = $gr_browse.SelectedPath
    
    $gr_browse.Dispose()

    return $gr_return          # OK or Cancel + SelectedPath

} # Function Get-RepoFolder

#=====================================================================================
<#
    Function Browse-IndividualRepos
...
Function Browse-IndividualRepos {
    ...
    # ask the user for the repository
    $bi_Ask = Get-RepoFolder $pCurrentRepository "Select a Root Folder for individual repositories"
    
    $bi_Repository = $pCurrentRepository                    # set default to existing repository path
    if ( $bi_Ask.Return -eq "OK" ) {
        $bi_Repository = $bi_Ask.SelectedPath
        Import-IndividualRepos $pGrid $bi_Repository        # call Import-IndividualRepos to populate the grid with models from this repository
    } # if ( $bi_Ask.Return -eq "OK" )
...
#=====================================================================================
<#
    Function Import-IndividualRepos
...
Function Import-IndividualRepos {
    [CmdletBinding()]
	param( $pGrid, $pCurrentRepository )
    if ( $v_DebugMode ) { CMTraceLog -Message "> Import-IndividualRepos START - at ($($pCurrentRepository))" -Type $TypeNorm }

    if ( $v_DebugMode ) { CMTraceLog -Message  "... calling Empty-Grid() to clear list" -Type $TypeNorm }
    Empty-Grid $pGrid
    if ( -not (Test-Path $pCurrentRepository) ) {
        Init-Repository $pCurrentRepository $False  # $false means this is not a HPIA repo, so don't initialize it as such
    }
    Set-Location $pCurrentRepository

    $ii_ReposFound = $False     # assume no repositories exist here

    if ( $v_DebugMode ) { CMTraceLog -Message  "... checking for existing repositories" -Type $TypeNorm }
    $ii_Directories = (Get-ChildItem -Path $pCurrentRepository -Directory) | Where-Object { $_.BaseName -notmatch "_BK$" }

    # search for repositories in each subfolder of the head repository folder
    foreach ( $iPath in $ii_Directories ) { 
        # handle PS5 vs PS7+ differences in how Get-ChildItem returns objects
        $ir_RepoFolder = "$($pCurrentRepository)\$($iPath)"         # this is the PS5 output
        $ir_RepoName = $iPath.BaseName     

        if ( $PSversionTable.PSVersion.Major -eq 7 ) { 
            $ir_RepoFolder = $iPath
            $ir_RepoName = Split-Path $iPath.FullName -Leaf
        }
        ...
        Try {
            ...
            [array]$ir_RepoPlatform = (Get-RepositoryInfo).Filters.platform | Get-Unique

            if ( $ir_RepoPlatform ) {
                # add the platforms found in the repository to the grid
                $ir_RepoPlatform | ForEach-Object {
                    [void]$pGrid.Rows.Add(@( $true, $_, $ir_RepoName ))                
                } # $ir_RepoPlatform | foreach           
                $ii_ReposFound = $True
            } else {
                CMTraceLog -Message  "... $($ir_RepoFolder) NO Filters in the Repository" -Type $TypeWarn
                # get the platform from the folder name
                $ii_ProdName = Split-Path $ir_RepoFolder -Leaf
                # let's try to get the platform from the folder name, if it is in the format SysID_ModelName
                [array]$ii_ProdID = Start-Job -ScriptBlock { Import-Module HPCMSL; Get-HPDeviceDetails -like $using:ii_ProdName } | Wait-Job | Receive-Job
                [void]$pGrid.Rows.Add(@( $true, $ii_ProdID[0].SystemID, $ir_RepoName ))
            }

        } Catch {
            CMTraceLog -Message  "... $($ir_RepoFolder) is NOT a Repository" -Type $TypeNorm
        } # Catch
    } # foreach ( $iFolder in $ii_Directories )
    
    if ( $ii_ReposFound ) {
        if ( $v_DebugMode ) { CMTraceLog -Message  "... > Get-IndividualRepofilters() - retrieving Filters" }
        Get-IndividualRepofilters $pGrid $pCurrentRepository
        if ( $v_DebugMode ) { CMTraceLog -Message  "... > Update-INIModelsFromGrid()" }
        Update-INIModelsFromGrid $pGrid $pCurrentRepository  
    } else {
        ...
        if ( $ir_askClear -eq 'Yes' ) {
            if ( $v_DebugMode ) { CMTraceLog -Message  "...calling Update-UIandINISetting() to clear HPModels list " }
            Update-UIandINISetting $pCurrentRepository $False              # $False = using individual repositories
        } else {
            CMTraceLog -Message  "... previous HPModels list remain in INI file"
        }
    } # else if ( $ii_ReposFound )

    if ( $v_DebugMode ) { CMTraceLog -Message '< Import-IndividualRepos DONE' -Type $TypeSuccess }
} # Function Import-IndividualRepos

#=====================================================================================
<#
    Function Browse-CommonRepo
...
Function Browse-CommonRepo {
    ...
    if ( $v_DebugMode ) { CMTraceLog -Message '> Browse-CommonRepo START' -Type $TypeNorm }
    # let's find the repository to import or use the current

    # ask the user for the repository
    $bc_Ask = Get-RepoFolder $pCurrentRepository "Select an HPIA Common/Shared Repository"

    $bc_Repository = $pCurrentRepository                           # set default to null in case user cancels
    if ( $bc_Ask.Return -eq "OK" ) {
        $bc_Repository = $bc_Ask.SelectedPath
        Import-Repository $pGrid $bc_Repository $pModelsTable        
    } # if ( $bi_Ask.Return -eq "OK" )
...
Function Import-Repository {
    ...
    if ( $v_DebugMode ) { CMTraceLog -Message "> Import-Repository: $($pRepoFolder)" -Type $TypeSuccess }
    Empty-Grid $pGrid
    if ( [string]::IsNullOrEmpty($pRepoFolder ) ) {
        ...
        if ( Test-Path $pRepoFolder ) {            
            Try {
                ...
                #$_ir_filterJob = Start-Job -ScriptBlock { Set-Location $using:pRepoFolder; Import-Module HPCMSL; Get-RepositoryInfo -ErrorAction Stop } | Wait-Job
                $_ir_filterJob = Get-RepositoryInfo -ErrorAction Stop
                if ( $_ir_filterJob.State -eq 'Failed' ) { throw 'Not a valid HPIA repository' }
                $ir_ProdFilters = ($_ir_filterJob).Filters
            } Catch {
                Init-Repository $pRepoFolder $true
            }
        } else {
            # Folder does not exist, so initialize it for HPIA
            write-host "... $($pRepoFolder) does not exist, initializing it for HPIA"
            Init-Repository $pRepoFolder $true
        ...
        Try {
            ...
            if ( $ir_ProdFilters.count -eq 0 ) {
                ...
                if ( $v_DebugMode ) { CMTraceLog -Message  "... Updating UI and INI file from filters - > Update-UIandINISetting()" -Type $TypeNorm }
                Update-UIandINISetting $pRepoFolder $True     # $True = common repo
                if ( $v_DebugMode ) { CMTraceLog -Message  "... Updating models in INI file - > Update-INIModelsFromGrid()" -Type $TypeNorm }
                Update-INIModelsFromGrid $pGrid $pRepoFolder  
            } else {
                #--------------------------------------------------------------------------------
                ...
                for ( $i = 0 ; $i -lt $ir_RepoPlatforms.count ; $i++) {
                    ...
                    if ( -not $ir_PlatformAdded ) {
                        [array]$ir_ProdName = Start-Job -ScriptBlock { Import-Module HPCMSL; Get-HPDeviceDetails -Platform $using:ir_SysID } | Wait-Job | Receive-Job
                        [void]$pGrid.Rows.Add(@( $false, $ir_SysID, $ir_ProdName[0].Name ))
                    } # if ( -not $ir_PlatformAdded )
                } # for ( $i = 0 ; $i -lt $ir_RepoPlatforms.count ; $i++)

                if ( $v_DebugMode ) { CMTraceLog -Message "... Finding filters in $($pRepoFolder) (calling Get-CommonRepofilters())" -Type $TypeNorm }
                Get-CommonRepofilters $pGrid $pRepoFolder

                if ( $v_DebugMode ) { CMTraceLog -Message  "... Updating UI and INI file from filters (calling Update-UIandINISetting())" -Type $TypeNorm }
                Update-UIandINISetting $pRepoFolder $True     # $True = common repo

                if ( $v_DebugMode ) { CMTraceLog -Message  "... Updating models in INI file (calling Update-INIModelsFromGrid())" -Type $TypeNorm }
                 Update-INIModelsFromGrid $pGrid $pRepoFolder  
...
#=====================================================================================
<#
    Function Test-PlatformsOSVersion
...
Function Test-PlatformsOSVersion  {
    ...
    if ( $v_DebugMode ) { CMTraceLog -Message "> Test-PlatformsOSVersion($($pOS)/$($pOSVersion))" -Type $TypeNorm }
    ...
    for ( $i = 0; $i -lt $pGrid.RowCount; $i++ ) {
        ...
        if ( $pGrid[0,$i].Value ) {
            ...
            $cp_OSList = Start-Job -ScriptBlock { Import-Module HPCMSL; Get-HPDeviceDetails -Platform $using:cp_Platform -OSList -ErrorAction Continue } | Wait-Job | Receive-Job
    ...
    if ( $v_DebugMode ) { CMTraceLog -Message '< Test-PlatformsOSVersion()' -Type $TypeNorm }

} # Function Test-PlatformsOSVersion

#=====================================================================================
<#
    Function Mod-INISetting
...
#=====================================================================================
<#
    Function Update-INIModelsFromGrid
...
Function Update-INIModelsFromGrid {
    ...
    if ( $v_DebugMode ) { CMTraceLog -Message '... > Update-INIModelsFromGrid()' -Type $TypeNorm }
...
#=====================================================================================
<#
    Function Update-UIandINISetting
...
Function Update-UIandINISetting {
    ...
    # ---------------------------------------------------------
    Mod-INISetting $IniFIleFullPath $find $replace "... Updating '$($Script:IniFile)' setting: ''$($replace)''"
    # ---------------------------------------------------------
...
Function Manage_AddOnFlagFile {
    ...
    # ---------------------------------------------------------------------------------------
    # build the flag file name path, and the file backup name (when AddOn col is unchecked)
    # ---------------------------------------------------------------------------------------
    switch ( $pCommonFlag ) {
    ...
    if ( $pUseFile ) {
        ...
        if ( -not (Test-Path $ma_FlagFilePath) ) {
            if ( Test-Path $ma_FlagFilePathBK ) {
                ...
                if ( $v_DebugMode ) { CMTraceLog -Message "...   calling New-AddOnFlagFile()" -Type $TypeNorm }
                New-AddOnFlagFile $pPath $pSysID $pModelName $pAddOns $pCommonFlag
            } # else if ( Test-Path $ma_FlagFilePathBK )
        } # if ( -not (Test-Path $ma_FlagFilePath) )
        
        if ( $v_DebugMode ) { CMTraceLog -Message "...   calling Edit-AddOnFlagFile() - for additional Softpaqs" -Type $TypeNorm }
        Edit-AddOnFlagFile $ma_FlagFilePath $pSysID
...
Function Edit-AddOnFlagFile { 
    ...
    $ea_AddOnsFileContent = Get-Content $pFlagFilePath

    $ea_EntryFormWidth = 400 ; $ea_EntryFormHeigth = 500 ; $ea_FieldOffset = 20 ; $ea_FieldHeight = 20 ; $ea_PathFieldLength = 200
    ...
    # -----------------------------------------------------------
    $ea_EntryComment = New-Object System.Windows.Forms.Label
    $ea_EntryComment.Text = "Search for Softpaq names to add to the list"
    $ea_EntryComment.ForeColor = 'blue'
    $ea_EntryComment.location = New-Object System.Drawing.Point($ea_FieldOffset,($ea_FieldOffset-15))        # (from left, from top)
    $ea_EntryComment.Size = New-Object System.Drawing.Size(300,20)                                       # (width, height)

    $ea_EntryIDLabel = New-Object System.Windows.Forms.Label
    $ea_EntryIDLabel.Text = "Name"
    $ea_EntryIDLabel.location = New-Object System.Drawing.Point($ea_FieldOffset,($ea_FieldOffset+10))        # (from left, from top)
    ...
    $ea_SoftpaqEntry.location = New-Object System.Drawing.Point(($ea_FieldOffset+70),($ea_FieldOffset+10)) # (from left, from top)
    ...
    $ea_SoftpaqMsg.location = New-Object System.Drawing.Point($ea_FieldOffset,($ea_EntryFormHeigth-340))   # (from left, from top)    
    $ea_SoftpaqMsg.Size = New-Object System.Drawing.Size(($ea_PathFieldLength+60), ($ea_FieldHeight+20))           # (width, height)    

    $ea_AddButton = New-Object System.Windows.Forms.Button
    $ea_AddButton.Location = New-Object System.Drawing.Point(($ea_PathFieldLength+$ea_FieldOffset+80),($ea_FieldOffset+8))
    ...
    $ea_AddButton_AddClick = {
        if ( $ea_SoftpaqEntry.Text ) {
            ...
            if ( -not $ea_SoftpaqListRetrieved ) {
                ...
                $_ea_os = $Script:v_OS; $_ea_osver = $Script:v_OSVER
                $ea_Softpaqs = Start-Job -ScriptBlock { Import-Module HPCMSL; Get-HPSoftpaqList -Platform $using:pPlatformID -os $using:_ea_os -OsVer $using:_ea_osver -ReferenceUrl 'https://hpia.hpcloud.hp.com/ref/' -ErrorAction SilentlyContinue } | Wait-Job | Receive-Job
            ...
            if ( $ea_SoftpaqList.Count -eq 0 ) {
                ...
                $ea_entryMatched = $ea_AddOnsFileContent | ForEach-Object { $_.Trim() } | Where-Object { $_ -match $ea_SoftpaqEntry.Text } # trim empty lines
                if ( $ea_entryMatched ) {
                ...
                } # else if ( $ea_SoftpaqEntry.Text -in $ea_AddOnsFileContent )
    ...
    $ea_EntryList.Size = New-Object System.Drawing.Size(($ea_EntryFormWidth-60),($ea_EntryFormHeigth/2-100)) # (width, height)
    # populate the ListBox with existing entries from the flag file
    $ea_AddOnsFileContent | foreach { $ea_EntryList.items.add($_)}
    ...
    $ea_removeButton.Location = New-Object System.Drawing.Point($ea_FieldOffset,(60 + ($ea_EntryFormHeigth/2-100) + 5))
    ...
    # -----------------------------------------------------------
    # Multi-select listbox at bottom of form
    # -----------------------------------------------------------
    $ea_AvailableLabel = New-Object System.Windows.Forms.Label
    $ea_AvailableLabel.Text = "Optionally, choose entries from ini file (multi-select):"
    $ea_AvailableLabel.location = New-Object System.Drawing.Point($ea_FieldOffset,250)
    $ea_AvailableLabel.Size = New-Object System.Drawing.Size(300,20)

    $ea_AvailableList = New-Object System.Windows.Forms.ListBox
    $ea_AvailableList.Name = 'AvailableSoftpaqs'
    $ea_AvailableList.SelectionMode = 'MultiExtended'
    $ea_AvailableList.Autosize = $false
    $ea_AvailableList.location = New-Object System.Drawing.Point($ea_FieldOffset,270)
    $ea_AvailableList.Size = New-Object System.Drawing.Size(($ea_EntryFormWidth-180),140)
    # Populate with items from Script:v_Softpaqs

    $Script:v_Softpaqs | Sort-Object Name | ForEach-Object { [void]$ea_AvailableList.Items.Add($_) }

    
    $ea_AddSelectedButton = New-Object System.Windows.Forms.Button
    $ea_AddSelectedButton.Location = New-Object System.Drawing.Point(($ea_EntryFormWidth-120),370)
    $ea_AddSelectedButton.Size = New-Object System.Drawing.Size(75,23)
    $ea_AddSelectedButton.Text = 'Add >>'
    $ea_AddSelectedButton.add_Click({
        if ( $ea_AvailableList.SelectedItems.Count -gt 0 ) {
            foreach ( $item in $ea_AvailableList.SelectedItems ) {
                # Check if item is not already in the entry list
                if ( -not $ea_EntryList.Items.Contains($item) ) {
                    $ea_EntryList.Items.Add($item)
                }
            }
            # Update the flag file with new entries
            $ea_entries = @()
            foreach ( $iEntry in $ea_EntryList.items ) { $ea_entries += $iEntry }
            Set-Content $pFlagFilePath -Value $ea_entries
        }
    }) # $ea_AddSelectedButton.add_Click
    
    # -----------------------------------------------------------
    $ea_okButton = New-Object System.Windows.Forms.Button
    $ea_okButton.Location = New-Object System.Drawing.Point(($ea_EntryFormWidth-120),430)
    ...
    $ea_SoftpaqsForm.Controls.AddRange(@($ea_EntryComment,$ea_EntryIDLabel,$ea_SoftpaqEntry,$ea_AddButton,$ea_EntryList,$SpqLabel,$AddSoftpaqList, $ea_removeButton, $ea_SoftpaqMsg, $ea_AvailableLabel, $ea_AvailableList, $ea_AddSelectedButton, $ea_okButton))
    # -------------------------------------------------------------------
...
#=====================================================================================
<#
    Function Remove-SinglePlatform
...
Function Remove-Platforms {
    ...
    if ( $rp_removeList ) {
        ...
        if ( (AskUser $rp_Msg 1) -eq 'Ok') {    # 1 = OKCancel
            ...
            for ( $irow = $pGrid.RowCount-1 ; $irow -ge 0 ; $irow-- ) {
                if ( $pGrid.Rows[$irow].Cells[0].Value ) {
                    Remove-SinglePlatform $pGrid $irow $pRepositoryFolder $pCommonFlag
                    #$pGrid.Rows.RemoveAt($irow) 
                } # if ( $pGrid.Rows[$irow].Cells[0].Value )
            } # for ( $irow = 0 ; $irow -lt $pGrid.RowCount ; $irow++ )            
            if ( $v_DebugMode ) { CMTraceLog -Message '... calling Update-INIModelsFromGrid()' -Type $TypeWarn }
            Update-INIModelsFromGrid $pGrid $pRepositoryFolder            
        } else {
            $rp_removeList = $null
...
Function Add-Platform {
    ...
    $ap_SearchButton_AddClick = {
        if ( $ap_EntryModel.Text ) {
            ...
            $_ap_searchText = $ap_EntryModel.Text
            $ap_Models = Start-Job -ScriptBlock { Import-Module HPCMSL; Get-HPDeviceDetails -Like -Name $using:_ap_searchText } | Wait-Job | Receive-Job
    ...
    if ($ap_Result -eq [System.Windows.Forms.DialogResult]::OK) {        
        ...
        $_ap_selectedItem = $ap_AddEntryList.SelectedItem
        [array]$ap_SelectedEntry = Start-Job -ScriptBlock { Import-Module HPCMSL; Get-HPDeviceDetails -Like -Name $using:_ap_selectedItem } | Wait-Job | Receive-Job
        ...
        if ( $ap_platformExists ) { 
            ...
            if ( $v_DebugMode ) { CMTraceLog -Message '... > New-AddOnFlagFile()' -Type $TypeNorm }
            $ap_numEntries = New-AddOnFlagFile $pRepositoryFolder $ap_SelectedSysID $ap_SelectedModel $ap_AddSoftpaqList.SelectedItems $pCommonFlag
...
Function MainForm {
    ...
    # HP Brand Colors
    $HPBlue = [System.Drawing.ColorTranslator]::FromHtml('#0096D6')
    $HPDarkBlue = [System.Drawing.ColorTranslator]::FromHtml('#0057A8')
    $HPLightGray = [System.Drawing.ColorTranslator]::FromHtml('#F5F5F5')
    $HPMediumGray = [System.Drawing.ColorTranslator]::FromHtml('#E0E0E0')
    ...
    $CM_form.Text = "HP Repository Downloader v$($ScriptVersion)"
    $CM_form.Width = $FormWidth ; $CM_form.height = $FormHeight ; $CM_form.Autosize = $true
    $CM_form.StartPosition = 'CenterScreen'
    $CM_form.BackColor = [System.Drawing.Color]::White
    $CM_form.Font = New-Object System.Drawing.Font('Segoe UI',9,[System.Drawing.FontStyle]::Regular)
    ...
    $OSList.Name = "OS"
    $OSList.FlatStyle = 'Flat'
    $OSList.BackColor = [System.Drawing.Color]::White
    ...
    $OSList.add_SelectedIndexChanged( {
            ...
            Mod-INISetting $IniFIleFullPath $find $replace "$($IniFile) - changing setting: ''$($replace)''"
    } ) # $OSList.add_SelectedIndexChanged()
    ...
    $OSVERComboBox.FlatStyle = 'Flat'
    $OSVERComboBox.BackColor = [System.Drawing.Color]::White
    ...
    $OSVERComboBox.add_SelectedIndexChanged( {
        ...
        Test-PlatformsOSVersion $dataGridView $Script:v_OS $Script:v_OSVER        
        $find = "^[\$]v_OSVER"
    ...
    $PathsGroupBox.ForeColor = $HPDarkBlue
    $PathsGroupBox.Font = New-Object System.Drawing.Font('Segoe UI',9,[System.Drawing.FontStyle]::Bold)
    ...
    $IndividualRadioButton.Add_Click( {
            ...
            Mod-INISetting $IniFIleFullPath $find $replace "$($IniFile) - INI setting set: ''$($replace)''"
            Import-IndividualRepos $dataGridView $IndividualPathTextField.Text            
    ...
    $individualBrowse.FlatStyle = 'Flat'
    $individualBrowse.BackColor = $ButtonColor
    $individualBrowse.ForeColor = $ButtonTextColor
    $individualBrowse.FlatAppearance.BorderSize = 0
    $individualBrowse_Click = {
        $Script:v_CommonRepo = $false
        $find = "^[\$]v_CommonRepo"
        $replace = "`$v_CommonRepo = `$$Script:v_CommonRepo" 
        Mod-INISetting $IniFIleFullPath $find $replace "$($IniFile) - changing setting: ''$($replace)''"
        $mf_Repository = Browse-IndividualRepos $dataGridView $IndividualPathTextField.Text
        if ( $mf_repository ) {
    ...
    $CommonRadioButton.Add_Click( {
        ...
        $replace = "`$v_CommonRepo = `$$Script:v_CommonRepo" 
        Mod-INISetting $IniFIleFullPath $find $replace "$($IniFile) - changing setting: ''$($replace)''"
        Import-Repository $dataGridView $CommonPathTextField.Text $Script:HPModelsTable
    ...
    $commonBrowse.FlatStyle = 'Flat'
    $commonBrowse.BackColor = $ButtonColor
    $commonBrowse.ForeColor = $ButtonTextColor
    $commonBrowse.FlatAppearance.BorderSize = 0
    $commonBrowse_Click = {
        $Script:v_CommonRepo = $True
        $find = "^[\$]v_CommonRepo"
        $replace = "`$v_CommonRepo = `$$Script:v_CommonRepo" 
        Mod-INISetting $IniFIleFullPath $find $replace "$($IniFile) - changing setting: ''$($replace)''"
        $mf_repository = Browse-CommonRepo $dataGridView $CommonPathTextField.Text $Script:HPModelsTable
        if ( $mf_repository ) {
    ...
    $dataGridView.GridColor = $HPMediumGray
    $dataGridView.ColumnHeadersDefaultCellStyle.BackColor = $HPBlue
    ...
    $dataGridView.AlternatingRowsDefaultCellStyle.BackColor = $HPLightGray
    ...
    #----------------------------------------------------------------------------------
    if ( $v_DebugMode ) { Write-Host 'creating Repo path column' }
    ...
    $CellOnClick = {
        ...
        if ( $column -in @(0, 3, 4, 5, 6, 7, 8) ) {      # col 7='AddOns'
            ...
            switch ( $column ) {
                ...
                Default {   # here to deal with clicking on a category or 'AddOns' cell 
                    ...
                    if ($column -eq 8 ) {       # col 8 = AddOns
                        if ( $Script:v_CommonRepo ) { $m_RepoPath = $CommonPathTextField.Text } else { $m_RepoPath = $IndividualPathTextField.Text }
                        Manage_AddOnFlagFile $m_RepoPath $datagridview[1,$row].value $datagridview[2,$row].value $null $CellNewState $Script:v_CommonRepo
                    }
    ...
    $CMModelsGroupBox.ForeColor = $HPDarkBlue
    $CMModelsGroupBox.Font = New-Object System.Drawing.Font('Segoe UI',9,[System.Drawing.FontStyle]::Bold)
    ...
    $RefreshGridButton.FlatStyle = 'Flat'
    $RefreshGridButton.BackColor = $ButtonColor
    $RefreshGridButton.ForeColor = $ButtonTextColor
    $RefreshGridButton.FlatAppearance.BorderSize = 0

    $RefreshGridButton_Click={
        #CMTraceLog -Message 'Pupulating HP Models List from INI file $HPModels' -Type $TypeNorm
        Empty-Grid $dataGridView
        Update-GridFromINI $dataGridView $Script:HPModelsTable
    } # $RefreshGridButton_Click={
    ...
    $ListFiltersdButton.FlatStyle = 'Flat'
    $ListFiltersdButton.BackColor = $ButtonColor
    $ListFiltersdButton.ForeColor = $ButtonTextColor
    $ListFiltersdButton.FlatAppearance.BorderSize = 0

    $ListFiltersdButton_Click={
        CMTraceLog -Message "Listing filters (calling list-filters())" -Type $TypeNorm
        list-filters
    } # $ListFiltersdButton_Click={
    ...
    $AddModelButton.FlatStyle = 'Flat'
    $AddModelButton.BackColor = $ButtonColor
    $AddModelButton.ForeColor = $ButtonTextColor
    ...
    $AddModelButton.add_Click( { 
        ...
        $mf_ModelArray = Add-Platform $DataGridView $Script:v_Softpaqs $mf_RepoPath $Script:v_CommonRepo
    ...
    $RemoveModelButton.FlatStyle = 'Flat'
    $RemoveModelButton.BackColor = $ButtonColor
    $RemoveModelButton.ForeColor = $ButtonTextColor
    $RemoveModelButton.FlatAppearance.BorderSize = 0
    $RemoveModelButton.add_Click( { 
        if ( $Script:v_CommonRepo ) { $mf_RepoPath = $CommonPathTextField.Text } else { $mf_RepoPath = $IndividualPathTextField.Text }
        (Remove-Platforms $DataGridView $mf_RepoPath $Script:v_CommonRepo) | ForEach-Object {
    ...
    $buttonSync.BackColor = $HPDarkBlue
    $buttonSync.ForeColor = $ButtonTextColor
    $buttonSync.FlatAppearance.BorderSize = 0
    $buttonSync.Font = New-Object System.Drawing.Font('Segoe UI',10,[System.Drawing.FontStyle]::Bold)
    ...
    $buttonSync.add_click( {
        ...
        if ( ($OSVERComboBox.SelectedItem -in $Script:v_OSVALID10) -or ($OSVERComboBox.SelectedItem -in $Script:v_OSVALID11) ) {
            ...
            $lCheckedListArray = @()
            for ($i = 0; $i -lt $dataGridView.RowCount; $i++) {
                if ($dataGridView[0,$i].Value) { $lCheckedListArray += $i } # if  
            } # for ($i = 0; $i -lt $dataGridView.RowCount; $i++)
            if ( $lCheckedListArray.Count -eq 0 ) {
                CMTraceLog -Message "No models selected for Sync" -Type $TypeWarn
                [System.Windows.Forms.MessageBox]::Show("No models selected for Sync. Please select at least one model.", "Sync Error", 0)
                return
            }           
            if ( $Script:v_CommonRepo ) {
                Sync-CommonRepository $dataGridView $CommonPathTextField.Text #$Script:s_ModelAdded
                if ( $Script:s_ModelAdded ) { Update-INIModelsFromGrid $dataGridView $CommonPathTextField }   # $True = head of common repository
            } else {
                sync-individualRepositories $dataGridView $IndividualPathTextField.Text $Script:s_ModelAdded
                if ( $Script:s_ModelAdded ) { Update-INIModelsFromGrid $dataGridView $IndividualPathTextField.Text } 
            } # else if ( $Script:v_CommonRepo )
...
######################################################################################################
    ...
    $CMGroupAll.BackColor = $HPMediumGray
    $CMGroupAll.Text = 'SCCM - Disconnected'
    $CMGroupAll.ForeColor = $HPDarkBlue
    $CMGroupAll.Font = New-Object System.Drawing.Font('Segoe UI',9,[System.Drawing.FontStyle]::Bold)
    ...
    $HPIAPathButton.FlatStyle = 'Flat'
    $HPIAPathButton.BackColor = $ButtonColor
    $HPIAPathButton.ForeColor = $ButtonTextColor
    $HPIAPathButton.FlatAppearance.BorderSize = 0
    ...
    $buttonBrowse.BackColor = $ButtonColor
    $buttonBrowse.ForeColor = $ButtonTextColor
...
######################################################################################################
    ...
    $ClearTextBox.FlatStyle = 'Flat'
    $ClearTextBox.BackColor = $ButtonColor
    $ClearTextBox.ForeColor = $ButtonTextColor
    $ClearTextBox.FlatAppearance.BorderSize = 0
    ...
    $TextBoxFontdButtonDec.FlatStyle = 'Flat'
    $TextBoxFontdButtonDec.BackColor = $ButtonColor
    $TextBoxFontdButtonDec.ForeColor = $ButtonTextColor
    $TextBoxFontdButtonDec.FlatAppearance.BorderSize = 0
    ...
    $TextBoxFontdButtonInc.FlatStyle = 'Flat'
    $TextBoxFontdButtonInc.BackColor = $ButtonColor
    $TextBoxFontdButtonInc.ForeColor = $ButtonTextColor
    $TextBoxFontdButtonInc.FlatAppearance.BorderSize = 0
    ...
    $TextBoxNetTestButton.FlatStyle = 'Flat'
    $TextBoxNetTestButton.BackColor = $ButtonColor
    $TextBoxNetTestButton.ForeColor = $ButtonTextColor
    $TextBoxNetTestButton.FlatAppearance.BorderSize = 0
    ...
    $TextBoxNetTestButton_Click={
        ...
        if ( $r.Count -eq 1 ) {
            CMTraceLog -Message " HTTPS:/$($r.RemoteAddress) : $($r.TcpTestSucceeded)" -Type $TypeWarn
        } else {
   ...
   $CM_form.Controls.Add($TextBoxNetTestButton)
    ...
    #----------------------------------------------------------------------------------
    $find = "^[\$]v_CommonRepo"
    write-host 'loading repository paths from INI file: ' -ForegroundColor DarkMagenta
    (Get-Content $IniFIleFullPath) | 
        Foreach-Object { 
            if ($_ -match $find) {               # we found the variable
                if ( $_ -match '\$true' ) { 
                    write-host "Common Repository Path from INI file: $($CommonPathTextField.Text)" -ForegroundColor DarkMagenta
                    $m_CommonPath = $CommonPathTextField.Text
                    if ( ([string]::IsNullOrEmpty($m_CommonPath)) -or (-not (Test-Path $m_CommonPath)) ) {
                        Write-Host "Common Repository Path from INI file not found: $($m_CommonPath) - Will Create" -ForegroundColor DarkMagenta
                        Init-Repository $m_CommonPath $True                 # $True = make it a HPIA repo
                        write-host "Common Repository Path created: $($m_CommonPath)" -ForegroundColor DarkMagenta
                    }   
                    write-host "Importing Common Repository Path from INI file: $($m_CommonPath)" -ForegroundColor DarkMagenta
                    Import-Repository $dataGridView $m_CommonPath $Script:HPModelsTable
                    ...
                    if ( [string]::IsNullOrEmpty($IndividualPathTextField.Text) ) {
                        ...
                        write-host "Individual Repository Path from INI file: $($IndividualPathTextField.Text)" -ForegroundColor DarkMagenta
                        Import-IndividualRepos $dataGridView $IndividualPathTextField.Text
                        $IndividualRadioButton.Checked = $true 
                        $IndividualPathTextField.BackColor = $BackgroundColor
                    }
    ...
    write-host '... done loading repository paths from INI file' -ForegroundColor DarkMagenta
    #----------------------------------------------------------------------------------
...
if ( $MyInvocation.BoundParameters.count -eq 0) {
    ...
    if ( $PSBoundParameters.Keys.Contains('ListFilters') ) { list-filters $Script:v_CommonRepo }
    ...
    if ( $PSBoundParameters.Keys.Contains('Sync') ) { Sync-Repos $dataGridView $Script:v_CommonRepo }
