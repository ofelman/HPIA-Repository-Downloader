...
$ScriptVersion = "2.02.00 (April 28, 2022)"
    ...
    if ( $lbrowse.ShowDialog() -eq "OK" ) {
        ...
        foreach ( $lFolder in $lDirectories ) {
            ...
            Try {
                ...
                [void]$pGrid.Rows.Add(@( $true, $lRepoPlatform[0], $lFolder ))
                $lReposFound = $True
            } Catch {
...
Function Import_IndividualRepos {
    ...
    # check to see if the current folder is a common repository, and return if it is
    Try {
        $ret = Get-RepositoryInfo
        CMTraceLog "... Can not use a common repository: $($pCurrentRepository)"  -Type $TypeWarn
        return
    }
    Catch {        
    }
...
Function Manage_AddOnFlag_File {
    ...
    if ( $pCommonFlag ) {
        ...
        ###########################################################
        # if the path does not include the SysID in the folder name
        # ... leave it alone
        $lRpoPath = $pPath+'\'+$pModelName
        if ( -not (Test-Path $lRpoPath) ) {
            $lRpoPath = $pPath+'\'+$pSysID+'_'+$pModelName
        }
        # If the path does not exist, we just added the model to the list,
        # so there is no flag file to manage yet
        if ( -not (Test-Path $lRpoPath) ) { # repository folder does not exist yet
            return
        }
