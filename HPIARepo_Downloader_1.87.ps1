...
$ScriptVersion = "1.87 (August 15, 2021)"
...
if ( $help ) {
    ...
    "`nHPIARepo_Downloader_1.87.ps1 [[-Sync] [-ListFilters] [-inifile .\<filepath>\HPIARepo_ini.ps1] [-RepoStyle common|individual] [-products '80D4,8549,8470'] [-NoIniSw] [-ShowActivityLog]]`n"
    ...
    "`nExample: HPIARepo_Downloader_1.87.ps1 .\<path>\HPIARepo_ini.ps1 Common '80D4,8549,8470'`n"    
...
$ModelAdded = $False                                 # set $True when user adds models to list in the UI
...
[CmdletBinding()]
    ...
    for ( $i = 0; $i -lt $pGrid.RowCount; $i++ ) {
        ...
        if ( $i -in $pCheckedItemsList ) {
            ...
            #--------------------------------------------------------------------------------
            download_softpaqs_by_name  $lTempRepoFolder $lModelID $pGrid.Rows[$i].Cells['AddOns'].Value
    ...
    if ( $pNewModels ) { Update_INIModels $pGrid $pCommonRepoFolder $True }
...
Function Sync_Common_Repository {
[CmdletBinding()]
	param( $pGrid,                                       # the grid of platforms
            $pCommonRepoFolder,
    ...
    init_repository $pCommonRepoFolder $true                   # make sure Main repo folder exists, or create it, init
    CMTraceLog -Message  "... Common repository selected: $($pCommonRepoFolder)" -Type $TypeNorm
    ...
    #--------------------------------------------------------------------------------
    # loop through every Model in the grid
    for ( $item = 0; $item -lt $pGrid.RowCount; $item++ ) {
        ...
        $lAddOns = $pGrid[7,$item].Value                 # column 7 has the AddOns checkmark
        ...
        if ( $item -in $pCheckedItemsList ) {
            ...
            Update_Repository_and_Grid $pCommonRepoFolder $lModelId $item $true    # $true means 'add filters'
            #--------------------------------------------------------------------------------
            # update SCCM Repository package, if user decided with the UI checkmark
            #--------------------------------------------------------------------------------
            if ( $Script:v_UpdateCMPackages ) {
                CM_RepoUpdate $lModelName $lModelId $pCommonRepoFolder
            }
            #--------------------------------------------------------------------------------
            # update specific Softpaqs as defined in INI file for the current model
            #--------------------------------------------------------------------------------
            download_softpaqs_by_name  $pCommonRepoFolder $lModelID $pGrid.Rows[$item].Cells['AddOns'].Value
        } else {
            ...
            if ( $lResponse -eq 'Yes' ) {
                Update_Repository_and_Grid $pCommonRepoFolder $lModelId $item $false   # $false means 'do not add filters, just remove them'
            }
        } # if ( $item -in $pCheckedItemsList )
    } # for ( $item = 0; $item -lt $pGrid.RowCount; $item++ )

    # we are done checking every model for filters, so now do a Softpaq Sync and cleanup
    Sync_and_Cleanup_Repository $pCommonRepoFolder

    if ( $pNewModels ) { Update_INIModels $pGrid $pCommonRepoFolder $False }   # $False = head of individual repos
...
Function Get_CommonRepofilters {
    ...
    foreach ( $filter in $lProdFilters ) {
        ...
        for ( $i = 0; $i -lt $pDataGrid.RowCount; $i++ ) {
            ...
            if ( $pRefreshGrid ) {
                if ( $lPlatform -eq $pDataGrid[1,$i].value ) {
    ...
    if ( $lbrowse.ShowDialog() -eq "OK" ) {
        ...
        foreach ( $lFolder in $lDirectories ) {
            ...
            Try {
                ...
                [void]$pGrid.Rows.Add(@( $true, $lRepoPlatforms, $lFolder ))
            } Catch {
        ...
        Update_INIModels $pGrid $lRepository $False  # $False = treat as head of individual repositories
...
Function Import_IndividualRepos {
    ...
    $lDirectories = (ls -Directory)                    # each subfolder is an Individual model repository
    ...
    if ( $lDirectories.count -eq 0 ) {
        ...
        foreach ( $lProdName in $lDirectories ) {
            ...
            Try {
                ...
                [void]$pGrid.Rows.Add(@( $true, $lRepoFilters.platform, $lProdName ))
            } Catch {
        ...
        Update_INIModels $pGrid $pCurrentRepository $False  # $False = this is for Individual repositories
...
Function Browse_CommonRepo {
    ...
    if ( $lbrowse.ShowDialog() -eq "OK" ) {
        ...
        Try {
            ...
            Update_INIModels $pGrid $lRepository $True  # $True = this is a common repository
...
Function Import_CommonRepo {
    ...
    Try {
        ...
        if ( $lProdFilters.count -eq 0 ) {
            CMTraceLog -Message  "... no filters found" -Type $TypeNorm
...
Function Update_INIModels {
    ...
    $lModelsList = @()
    for ( $i = 0; $i -lt $pGrid.RowCount; $i++ ) {
        $lModelId = $pGrid[1,$i].Value             # column 1 has the Platform ID
        $lModelName = $pGrid[2,$i].Value           # column 2 has the Model name
        # add AddOns entries
        if ( $pCommonFlag ) {
            $lAddOnsFlagFile = $pRepositoryFolder+'\'+$addsoftware+'\'+$lModelId
        } else {
            $lAddOnsFlagFile = $pRepositoryFolder+'\'+$lModelName+'\'+$addsoftware+'\'+$lModelId
        }
        # create an array string of softpaq names to add to the platform entry in the models list
        $lAddModel = "`n`t@{ ProdCode = '$($lModelId)'; Model = '$($lModelName)' }"  # let's default the entry
        if (Test-Path $lAddOnsFlagFile) {
            ...
            if ( -not ([String]::IsNullOrWhiteSpace($lAddOns)) ) {
...
<#
    Function Manage_IDFlag_File
        Creates and updates the Pltaform ID flag file in the repository
    Parameters
...
Function Manage_IDFlag_File {
    ...
    if ( $Script:v_CommonRepo ) {           # copute the flag file path (from common repo or individual)
        $lFlagFile = $pPath+'\'+$AddSoftware+'\'+$pSysID
    } else {
        $lFlagFile = $pPath+'\'+$pModelName+'\'+$AddSoftware+'\'+$pSysID
    }
    if ( $pCreateFile ) {
        $lMsg = "... Setting up download of AddOns Softpaqs for Platform: $pSysID"
        CMTraceLog -Message $lMsg -Type $TypeNorm
        if ( Test-Path $lFlagFile ) { Clear-Content -Path $lFlagFile } else { New-Item $lFlagFile }

        $lSpqList = Get-SoftpaqList -platform $pSysID -os $OS -osver $Script:v_OSVER -characteristic SSM
            <#   --- Softpaq List contains entries like:
            Id           : sp118358
            ...
            SSM          : true
            DPB          : true
            Url          : ftp.hp.com/pub/softpaq/sp118001-118500/sp118358.exe
            ReleaseNotes : ftp.hp.com/pub/softpaq/sp118001-118500/sp118358.html
            Metadata     : ftp.hp.com/pub/softpaq/sp118001-118500/sp118358.cva
            MD5          : ba20d62aa49447b22ca9bbc68aed99b2
            Size         : 84419408
            ReleaseDate  : 2021-07-07
            UWP          : False
            #> 
        $lSupportedNamesList = @(($lSpqList).Name)    # create an array of softpaq 'names'

        #------------------------------------------------------------
        For ($i=0; $i -lt $pAddOns.count; $i++) {   # make sure every item selected is available for platform
            $lMatched = $False
            $lSoftpaq = $pAddOns[$i]
            for ($iEntry=0; $iEntry -lt $lSupportedNamesList.count; $iEntry++) {
                $lAddedSoftpaq = $lSupportedNamesList[$iEntry]
                if ( "$($lAddedSoftpaq)" -like "*$($lSoftpaq)*" ) {
                    #$lAddedSoftpaq = $lSupportedNamesList[$iEntry]
                    $lAddedSoftpaq | Out-File -FilePath $lFlagFile -Append
                    CMTraceLog -Message "... '$lAddedSoftpaq' added to AddOns list" -Type $TypeNorm
                    $lMatched = $True
                    break
                }
            } # for ($iEntry=0; $iEntry -lt $lSupportedNamesList.count; $iEntry++)
            if ( -not $lMatched ) { 
                CMTraceLog -Message "... '$lSoftpaq' not found in Softpaq list" -Type $TypeWarn
            }
        } # For ($i=0; $i -le $pAddOns.count; $i++)
        #------------------------------------------------------------
    } else {
...
Function AddModelForm {
    ...
    $EntryForm.Width = $EntryFormWidth ; $EntryForm.height = 400 ; $EntryForm.Autosize = $true
    $EntryForm.StartPosition = 'CenterScreen'
    $EntryForm.Topmost = $true

    $EntryId = New-Object System.Windows.Forms.Label
    $EntryId.Text = "Name"
    $EntryId.location = New-Object System.Drawing.Point($Offset,$Offset) # (from left, from top)
    ...
    $EntryModel.location = New-Object System.Drawing.Point(($Offset+70),($Offset-4)) # (from left, from top)
    ...
    $SearchButton.Location = New-Object System.Drawing.Point(($PathFieldLength+$Offset+80),($Offset-6))
    ...
    $lListHeight = $EntryFormHeigth/2-60
    $AddEntryList = New-Object System.Windows.Forms.ListBox
    $AddEntryList.Name = 'Entries'
    $AddEntryList.Autosize = $false
    $AddEntryList.location = New-Object System.Drawing.Point($Offset,60)  # (from left, from top)
    $AddEntryList.Size = New-Object System.Drawing.Size(($EntryFormWidth-60),$lListHeight) # (width, height)
    ...
    $SpqLabel.location = New-Object System.Drawing.Point($Offset,($lListHeight+60)) # (from left, from top)
    ...
    $AddSoftpaqList.location = New-Object System.Drawing.Point(($Offset+70),($lListHeight+60))  # (from left, from top)
    $AddSoftpaqList.Size = New-Object System.Drawing.Size(($EntryFormWidth-130),($lListHeight-40)) # (width, height)

    $okButton = New-Object System.Windows.Forms.Button
    $okButton.Location = New-Object System.Drawing.Point(($EntryFormWidth-120),($EntryFormHeigth-80))
    ...
    if ($lResult -eq [System.Windows.Forms.DialogResult]::OK) {
        ...
        for ( $i=0 ; $i -lt $lSelectedEntry.count ; $i++ ) {
            ...
            Manage_IDFlag_File $pRepositoryFolder $lSelectedEntry[$i].SystemID $lSelectedEntry[$i].Name $AddSoftpaqList.SelectedItems $True
            Update_INIModels $pGrid $pRepositoryFolder $pCommonFlag
        }
...
Function CreateForm {
    ...
    $AddModelButton.add_Click( { 
        ...
        if ( ($lModelArray = AddModelForm $DataGridView $Script:v_Softpaqs $lTempPath $Script:v_CommonRepo ) -ne $null ) {
            $Script:ModelAdded = $True
    ...
    $buttonSync.add_click( {
        ...
        if ( $Script:v_OSVER -in $Script:v_OSVALID ) {
            ...
            if ( $Script:v_CommonRepo ) {
                Sync_Common_Repository $dataGridView $CommonPathTextField.Text $lCheckedListArray $Script:ModelAdded
            } else {
                sync_individual_repositories $dataGridView $SharePathTextField.Text $lCheckedListArray $Script:ModelAdded
            }
            $Script:ModelAdded = $False        # reset as the previous Sync also updated INI file
        } # if ( $Script:v_OSVER -in $Script:v_OSVALID )
