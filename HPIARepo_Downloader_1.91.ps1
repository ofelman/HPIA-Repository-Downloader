<#
    ...
    by Dan Felman/HP Technical Consultant
        ...
        Version informtion in release_notes
#>
<#
    Args
...
$ScriptVersion = "1.91.01 (January 10, 2022)"
...
[CmdletBinding()] 
    ...
    if ( $pCommonRepoFlag ) {
        ...
        for ( $i = 0; $i -lt $pGrid.RowCount; $i++ ) {
            ...
            if ( $lFoldername -match $lModelName ) {                 # we found the mode, so get the Prod ID
...
[CmdletBinding()]
    ...
    foreach ( $cat in $Script:v_FilterCategories ) {
        if ( $pGrid.Rows[$pRow].Cells[$cat].Value ) {
            ...
            $lRes = (Add-RepositoryFilter -platform $pModelID -os win10 -osver $Script:v_OSVER -category $cat -characteristic ssm 6>&1)
...
param( $pGrid, $pPath )
    ...
    for ( $i = 0; $i -lt $pGrid.RowCount; $i++ ) {
        ...
        $lRes = (Add-RepositoryFilter -platform $lSysId -os win10 -osver $Script:v_OSVER -category BIOS -characteristic ssm 6>&1)
...
Function Import_IndividualRepos {
    ...
    CMTraceLog -Message  "... clearing Grid" -Type $TypeNorm
    Empty_Grid $pGrid
    ...
    if ( $lDirectories.count -eq 0 ) {
        ...
        foreach ( $ientry in $lDirectories ) {
            ...
            #if ( [string]$lentry.name.startswith("0-9") ) {
            If ([string]$ientry.name -match "^[0-9].*") {
                # if folder name starts with an Integer, it has the SysID in the name, so pick model name after SysID
                $lProdName = $ientry.name.substring(5)
            } else {
                $lProdName = $ientry.name
            }
            $lentrySysID = (Get-HPDeviceDetails -Name $lProdName).SystemID
            ...
            Try {
                ...
                if ( $lRepoFilters.count -eq 0 ) {
                    # get the platform ID from the AddOns flag file... this fikder should have a single flag file
                    $lPlatformID = Get-ChildItem $lRepoFolderFullPath"\.ADDSOFTWARE"
                    $lRow = $pGrid.Rows.Add(@( $False, $lPlatformID.Name.Substring(0,4), $lProdName ))
                } else {
                    $lRow = $pGrid.Rows.Add(@( $True, $lentrySysID, $lProdName ))
                }
                # --------------------------------------------------------------
                # now check for the AddOns flag file, and its contents
                # --------------------------------------------------------------
                $lAddOnsFlagFile = $lRepoFolderFullPath+'\.ADDSOFTWARE\'+$lentrySysID
...
Function Manage_AddOnFlag_File {
    ...
    if ( $pCommonFlag ) {
        ...
        $lFlagFilePath = $pPath+'\'+$pSysID+'_'+$pModelName+'\'+$s_AddSoftware+'\'+$pSysID
    }
    ...
    if ( $pCreateFile ) {
        ...
        CMTraceLog -Message "... Selecting additional Softpaqs for model $pModelName" -Type $TypeNorm
...
<#
        ...
        for ( $i=0 ; $i -lt $lSelectedEntry.count ; $i++ ) {
            ...
            # Let's also create a Model folder, if required for individual repositories
            if ( -not $pCommonFlag ) {
                $lnewPath = $pRepositoryFolder+'\'+$lSelectedEntry[$i].Name
                init_repository $lnewPath $true
            } # if ( -not $pCommonFlag )

            if ( ($lSelectedSoftpaqs = $AddSoftpaqList.SelectedItems).Count -gt 0 ){
                Manage_AddOnFlag_File $pRepositoryFolder $lSelectedEntry[$i].SystemID $lSelectedEntry[$i].Name $lSelectedSoftpaqs $True
            } else {
                Manage_AddOnFlag_File $pRepositoryFolder $lSelectedEntry[$i].SystemID $lSelectedEntry[$i].Name $Null $True
            }
            Update_INIModelsList $pGrid $pRepositoryFolder $pCommonFlag
        }
#>
