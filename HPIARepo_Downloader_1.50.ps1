...
Function Get_HPServerIP {
    ...
    start-job -Name $pJobName -scriptblock { 
        ...
        foreach ($connection in (Get-NetTCPConnection -appliedsetting internet)) {
        ...
        ForEach-Object {
            if ( $_.ProdCode -match $pProdCode ) {                              # let's match the Model to download for
                ...
                ForEach ( $Softpaq in $_.SqName ) {
                    ...
                    Catch {
                        CMTraceLog -Message  "... Get-SoftpaqList: $($lListret)" -Type $TypeNoNewLine
                        CMTraceLog -Message  "... 'exception -platform:$($pProdCode) -osver:$($OSVER) - Filter NOT Supported!" -Type $TypeError
                    }
